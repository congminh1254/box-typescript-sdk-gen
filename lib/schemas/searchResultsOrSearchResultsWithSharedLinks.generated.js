...
function deserializeSearchResultsOrSearchResultsWithSharedLinks(val) {
    if (!(0, json_js_1.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"SearchResultsOrSearchResultsWithSharedLinks\"" });
    }
    if (val.type == "search_results_items") {
    ...
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize SearchResultsOrSearchResultsWithSharedLinks" });
}
