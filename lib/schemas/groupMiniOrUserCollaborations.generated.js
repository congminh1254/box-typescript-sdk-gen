...
function deserializeGroupMiniOrUserCollaborations(val) {
    if (!(0, json_js_1.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"GroupMiniOrUserCollaborations\"" });
    }
    if (val.type == "group") {
    ...
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize GroupMiniOrUserCollaborations" });
}
