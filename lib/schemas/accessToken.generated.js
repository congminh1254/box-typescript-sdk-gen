...
function deserializeAccessTokenTokenTypeField(val) {
    ...
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize AccessTokenTokenTypeField" });
}
...
function deserializeAccessTokenIssuedTokenTypeField(val) {
    ...
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize AccessTokenIssuedTokenTypeField" });
}
function serializeAccessToken(val) {
    return { ["access_token"]: val.accessToken, ["expires_in"]: val.expiresIn, ["token_type"]: val.tokenType == void 0 ? val.tokenType : serializeAccessTokenTokenTypeField(val.tokenType), ["restricted_to"]: val.restrictedTo == void 0 ? val.restrictedTo : val.restrictedTo.map(function (item) {
            return (0, fileOrFolderScope_generated_js_1.serializeFileOrFolderScope)(item);
        }), ["refresh_token"]: val.refreshToken, ["issued_token_type"]: val.issuedTokenType == void 0 ? val.issuedTokenType : serializeAccessTokenIssuedTokenTypeField(val.issuedTokenType) };
}
function deserializeAccessToken(val) {
    ...
    if (!(val.access_token == void 0) && !(0, json_js_2.sdIsString)(val.access_token)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"access_token\" of type \"AccessToken\"" });
    }
    ...
    if (!(val.expires_in == void 0) && !(0, json_js_1.sdIsNumber)(val.expires_in)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting number for \"expires_in\" of type \"AccessToken\"" });
    }
    ...
    if (!(val.restricted_to == void 0) && !(0, json_js_3.sdIsList)(val.restricted_to)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting array for \"restricted_to\" of type \"AccessToken\"" });
    }
    const restrictedTo = val.restricted_to == void 0 ? void 0 : (0, json_js_3.sdIsList)(val.restricted_to) ? val.restricted_to.map(function (itm) {
    ...
    if (!(val.refresh_token == void 0) && !(0, json_js_2.sdIsString)(val.refresh_token)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"refresh_token\" of type \"AccessToken\"" });
    }
