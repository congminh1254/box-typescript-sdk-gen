...
function deserializeIntegrationMappingPartnerItemSlackUnion(val) {
    if (!(0, json_js_1.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"IntegrationMappingPartnerItemSlackUnion\"" });
    }
    if (val.type == "channel") {
        return (0, integrationMappingPartnerItemSlack_generated_js_2.deserializeIntegrationMappingPartnerItemSlack)(val);
    }
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize IntegrationMappingPartnerItemSlackUnion" });
}
