...
function deserializeEventsNextStreamPositionField(val) {
    ...
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize EventsNextStreamPositionField" });
}
function serializeEvents(val) {
    return { ["chunk_size"]: val.chunkSize, ["next_stream_position"]: val.nextStreamPosition == void 0 ? val.nextStreamPosition : serializeEventsNextStreamPositionField(val.nextStreamPosition), ["entries"]: val.entries == void 0 ? val.entries : val.entries.map(function (item) {
...
function deserializeEvents(val) {
    ...
    if (!(val.chunk_size == void 0) && !(0, json_js_1.sdIsNumber)(val.chunk_size)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting number for \"chunk_size\" of type \"Events\"" });
    }
    const chunkSize = val.chunk_size == void 0 ? void 0 : val.chunk_size;
    const nextStreamPosition = val.next_stream_position == void 0 ? void 0 : deserializeEventsNextStreamPositionField(val.next_stream_position);
    if (!(val.entries == void 0) && !(0, json_js_3.sdIsList)(val.entries)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting array for \"entries\" of type \"Events\"" });
    }
