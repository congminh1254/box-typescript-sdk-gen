...
function deserializeFileVersionLegalHoldTypeField(val) {
    ...
    throw new errors_js_1.BoxSdkError({ message: "Can't deserialize FileVersionLegalHoldTypeField" });
}
function serializeFileVersionLegalHold(val) {
    return { ["id"]: val.id, ["type"]: val.type == void 0 ? val.type : serializeFileVersionLegalHoldTypeField(val.type), ["file_version"]: val.fileVersion == void 0 ? val.fileVersion : (0, fileVersionMini_generated_js_1.serializeFileVersionMini)(val.fileVersion), ["file"]: val.file == void 0 ? val.file : (0, fileMini_generated_js_1.serializeFileMini)(val.file), ["legal_hold_policy_assignments"]: val.legalHoldPolicyAssignments == void 0 ? val.legalHoldPolicyAssignments : val.legalHoldPolicyAssignments.map(function (item) {
            return (0, legalHoldPolicyAssignment_generated_js_1.serializeLegalHoldPolicyAssignment)(item);
        }), ["deleted_at"]: val.deletedAt == void 0 ? val.deletedAt : (0, utils_js_1.serializeDateTime)(val.deletedAt) };
}
function deserializeFileVersionLegalHold(val) {
    if (!(0, json_js_3.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"FileVersionLegalHold\"" });
    }
    if (!(val.id == void 0) && !(0, json_js_1.sdIsString)(val.id)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"id\" of type \"FileVersionLegalHold\"" });
    }
    ...
    if (!(val.legal_hold_policy_assignments == void 0) && !(0, json_js_2.sdIsList)(val.legal_hold_policy_assignments)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting array for \"legal_hold_policy_assignments\" of type \"FileVersionLegalHold\"" });
    }
    const legalHoldPolicyAssignments = val.legal_hold_policy_assignments == void 0 ? void 0 : (0, json_js_2.sdIsList)(val.legal_hold_policy_assignments) ? val.legal_hold_policy_assignments.map(function (itm) {
    ...
    if (!(val.deleted_at == void 0) && !(0, json_js_1.sdIsString)(val.deleted_at)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"deleted_at\" of type \"FileVersionLegalHold\"" });
    }
