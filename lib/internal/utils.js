...
class Hash {
    constructor({ algorithm }) {
        ...
        if (isBrowser()) {
            ...
            __classPrivateFieldSet(this, _Hash_hash, evalRequire('crypto').createHash(algorithm), "f");
        }
    }
...
function generateByteStreamFromBuffer(buffer) {
    return isBrowser()
        ...
        : evalRequire('stream').Readable.from(buffer_1.Buffer.from(buffer));
}
...
function decodeBase64ByteStream(data) {
    return isBrowser()
        ...
        : evalRequire('stream').Readable.from(buffer_1.Buffer.from(data, 'base64'));
}
function stringToByteStream(data) {
    return isBrowser()
        ...
        : evalRequire('stream').Readable.from(buffer_1.Buffer.from(data, 'ascii'));
}
