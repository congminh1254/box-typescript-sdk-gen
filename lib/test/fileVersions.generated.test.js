...
test('testCreateListGetPromoteFileVersion', function testCreateListGetPromoteFileVersion() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const fileVersions = yield client.fileVersions.getFileVersions(file.id);
        if (!(fileVersions.totalCount == 1)) {
            throw new Error('Assertion failed');
        }
        const fileVersion = yield client.fileVersions.getFileVersionById(file.id, fileVersions.entries[0].id);
        if (!(fileVersion.id == fileVersions.entries[0].id)) {
            throw new Error('Assertion failed');
        }
        yield client.fileVersions.promoteFileVersion(file.id, {
            requestBody: {
                id: fileVersions.entries[0].id,
                type: 'file_version',
            },
        });
        const fileWithPromotedVersion = yield client.files.getFileById(file.id);
        ...
        yield client.fileVersions.deleteFileVersionById(file.id, fileVersion.id);
...
test('testRemoveAndRestoreFileVersion', function testRemoveAndRestoreFileVersion() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const file = files.entries[0];
        yield client.uploads.uploadFileVersion(file.id, {
            attributes: {
                name: newName,
            },
            file: generateByteStream(20),
        ...
        yield client.fileVersions.deleteFileVersionById(file.id, fileVersion.id);
        const deletedFileVersion = yield client.fileVersions.getFileVersionById(file.id, fileVersions.entries[0].id, {
            queryParams: {
                fields: [
                    'trashed_at',
                    'trashed_by',
        ...
        yield client.fileVersions.updateFileVersionById(file.id, fileVersion.id, {
            requestBody: {
                trashedAt: createNull(),
            },
        });
        const restoredFileVersion = yield client.fileVersions.getFileVersionById(file.id, fileVersions.entries[0].id, {
            queryParams: {
                fields: [
                    'trashed_at',
                    'trashed_by',
