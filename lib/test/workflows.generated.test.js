...
test("testWorkflows", function testWorkflows() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const uploadedFiles = yield adminClient.uploads.uploadFile({ attributes: { name: (0, utils_js_1.getUuid)(), parent: { id: workflowFolderId } }, file: (0, utils_js_3.generateByteStream)(1024 * 1024) });
        const file = uploadedFiles.entries[0];
        const workflowFileId = file.id;
        const workflows = yield adminClient.workflows.getWorkflows({ folderId: workflowFolderId });
        ...
        if (!((0, utils_js_4.toString)(workflowToRun.flows[0].trigger.triggerType) == "WORKFLOW_MANUAL_START")) {
            throw new Error("Assertion failed");
        }
        if (!((0, utils_js_4.toString)(workflowToRun.flows[0].outcomes[0].actionType) == "delete_file")) {
            throw new Error("Assertion failed");
        }
        if (!((0, utils_js_4.toString)(workflowToRun.flows[0].outcomes[0].type) == "outcome")) {
            throw new Error("Assertion failed");
        }
        yield adminClient.workflows.startWorkflow(workflowToRun.id, { type: "workflow_parameters", flow: { type: "flow", id: workflowToRun.flows[0].id }, files: [{ type: "file", id: workflowFileId }], folder: { type: "folder", id: workflowFolderId } });
    });
});
