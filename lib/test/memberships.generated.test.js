...
test("testMemberships", function testMemberships() {
    return __awaiter(this, void 0, void 0, function* () {
        const user = yield exports.client.users.createUser({ name: (0, utils_js_1.getUuid)(), login: "".concat((0, utils_js_1.getUuid)(), "@boxdemo.com") });
        const userMemberships = yield exports.client.memberships.getUserMemberships(user.id);
        if (!(userMemberships.totalCount == 0)) {
            throw new Error("Assertion failed");
        }
        const group = yield exports.client.groups.createGroup({ name: (0, utils_js_1.getUuid)() });
        const groupMemberships = yield exports.client.memberships.getGroupMemberships(group.id);
        if (!(groupMemberships.totalCount == 0)) {
            throw new Error("Assertion failed");
        }
        const groupMembership = yield exports.client.memberships.createGroupMembership({ user: { id: user.id }, group: { id: group.id } });
        ...
        const updatedGroupMembership = yield exports.client.memberships.updateGroupMembershipById(groupMembership.id, { requestBody: { role: "admin" } });
        ...
        yield expect(() => __awaiter(this, void 0, void 0, function* () { yield exports.client.memberships.getGroupMembershipById(groupMembership.id); })).rejects.toThrow();
