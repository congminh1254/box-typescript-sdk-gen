...
test("testGetTermsOfServiceUserStatuses", function testGetTermsOfServiceUserStatuses() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const user = yield client.users.createUser({ name: (0, utils_js_1.getUuid)(), login: "".concat((0, utils_js_1.getUuid)(), "@boxdemo.com"), isPlatformAccessOnly: true });
        const createdTosUserStatus = yield client.termsOfServiceUserStatuses.createTermsOfServiceStatusForUser({ tos: new termsOfServiceUserStatuses_generated_js_1.CreateTermsOfServiceStatusForUserRequestBodyTosField({ id: tos.id }), user: new termsOfServiceUserStatuses_generated_js_2.CreateTermsOfServiceStatusForUserRequestBodyUserField({ id: user.id }), isAccepted: false });
        if (!(createdTosUserStatus.isAccepted == false)) {
            throw new Error("Assertion failed");
        }
        if (!((0, utils_js_3.toString)(createdTosUserStatus.type) == "terms_of_service_user_status")) {
            throw new Error("Assertion failed");
        }
        ...
        const updatedTosUserStatus = yield client.termsOfServiceUserStatuses.updateTermsOfServiceStatusForUserById(createdTosUserStatus.id, { isAccepted: true });
        if (!(updatedTosUserStatus.isAccepted == true)) {
            throw new Error("Assertion failed");
        }
        const listTosUserStatuses = yield client.termsOfServiceUserStatuses.getTermsOfServiceUserStatuses({ tosId: tos.id, userId: user.id });
