...
test("testCreateUpdateGetDeleteRetentionPolicyAssignment", function testCreateUpdateGetDeleteRetentionPolicyAssignment() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const retentionPolicy = yield exports.client.retentionPolicies.createRetentionPolicy({ policyName: retentionPolicyName, policyType: "finite", areOwnersNotified: true, canOwnerExtendRetention: true, description: retentionDescription, dispositionAction: "remove_retention", retentionLength: "1", retentionType: "modifiable" });
        const folder = yield exports.client.folders.createFolder({ name: (0, utils_js_1.getUuid)(), parent: { id: "0" } });
        const files = yield exports.client.uploads.uploadFile({ attributes: { name: (0, utils_js_1.getUuid)(), parent: { id: folder.id } }, file: (0, utils_js_2.generateByteStream)(10) });
        const file = files.entries[0];
        const newVersions = yield exports.client.uploads.uploadFileVersion(file.id, { attributes: { name: (0, utils_js_1.getUuid)() }, file: (0, utils_js_2.generateByteStream)(20) });
        const newVersion = newVersions.entries[0];
        const retentionPolicyAssignment = yield exports.client.retentionPolicyAssignments.createRetentionPolicyAssignment({ policyId: retentionPolicy.id, assignTo: { type: "folder", id: folder.id } });
