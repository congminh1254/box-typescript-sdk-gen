...
class BoxJwtAuth {
    ...
    refreshToken(networkSession) {
        return __awaiter(this, void 0, void 0, function* () {
            if ((0, utils_js_3.isBrowser)()) {
                throw new errors_js_1.BoxSdkError({ message: "JWT auth is not supported in browser environment." });
            }
            const alg = !(this.config.algorithm == void 0) ? this.config.algorithm : "RS256";
            const claims = { ["exp"]: (0, utils_js_4.getEpochTimeInSeconds)() + 30, ["box_sub_type"]: this.subjectType };
            const jwtOptions = { algorithm: alg, audience: "https://api.box.com/oauth2/token", subject: this.subjectId, issuer: this.config.clientId, jwtid: (0, utils_js_1.getUuid)(), keyid: this.config.jwtKeyId };
            const jwtKey = { key: this.config.privateKey, passphrase: this.config.privateKeyPassphrase };
            const assertion = yield (0, utils_js_5.createJwtAssertion)(claims, jwtKey, jwtOptions);
            const authManager = new authorization_generated_js_1.AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession : new network_generated_js_1.NetworkSession({}) });
            const token = yield authManager.requestAccessToken({ grantType: "urn:ietf:params:oauth:grant-type:jwt-bearer", assertion: assertion, clientId: this.config.clientId, clientSecret: this.config.clientSecret });
    ...
    withUserSubject(userId, tokenStorage = new tokenStorage_generated_js_1.InMemoryTokenStorage({})) {
        const newConfig = new JwtConfig({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, enterpriseId: void 0, userId: userId, jwtKeyId: this.config.jwtKeyId, privateKey: this.config.privateKey, privateKeyPassphrase: this.config.privateKeyPassphrase, tokenStorage: tokenStorage });
    ...
    withEnterpriseSubject(enterpriseId, tokenStorage = new tokenStorage_generated_js_1.InMemoryTokenStorage({})) {
        const newConfig = new JwtConfig({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, enterpriseId: enterpriseId, userId: void 0, jwtKeyId: this.config.jwtKeyId, privateKey: this.config.privateKey, privateKeyPassphrase: this.config.privateKeyPassphrase, tokenStorage: tokenStorage });
        const newAuth = new BoxJwtAuth({ config: newConfig });
        return newAuth;
    }
    /**
    ...
    downscopeToken(scopes, resource, sharedLink, networkSession) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (token == void 0) {
                throw new errors_js_1.BoxSdkError({ message: "No access token is available. Make an API call to retrieve a token before calling this method." });
            }
            const authManager = new authorization_generated_js_1.AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession : new network_generated_js_1.NetworkSession({}) });
            const downscopedToken = yield authManager.requestAccessToken({ grantType: "urn:ietf:params:oauth:grant-type:token-exchange", subjectToken: token.accessToken, subjectTokenType: "urn:ietf:params:oauth:token-type:access_token", resource: resource, scope: scopes.join(" "), boxSharedLink: sharedLink });
    ...
    revokeToken(networkSession) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const authManager = new authorization_generated_js_1.AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession : new network_generated_js_1.NetworkSession({}) });
            yield authManager.revokeAccessToken({ token: oldToken.accessToken, clientId: this.config.clientId, clientSecret: this.config.clientSecret });
...
function deserializeJwtConfigAppSettingsAppAuth(val) {
    if (!(0, json_js_3.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"JwtConfigAppSettingsAppAuth\"" });
    }
    if (val.publicKeyID == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"publicKeyID\" of type \"JwtConfigAppSettingsAppAuth\" to be defined" });
    }
    if (!(0, json_js_2.sdIsString)(val.publicKeyID)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"publicKeyID\" of type \"JwtConfigAppSettingsAppAuth\"" });
    }
    const publicKeyId = val.publicKeyID;
    if (val.privateKey == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"privateKey\" of type \"JwtConfigAppSettingsAppAuth\" to be defined" });
    }
    if (!(0, json_js_2.sdIsString)(val.privateKey)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"privateKey\" of type \"JwtConfigAppSettingsAppAuth\"" });
    }
    const privateKey = val.privateKey;
    if (val.passphrase == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"passphrase\" of type \"JwtConfigAppSettingsAppAuth\" to be defined" });
    }
    if (!(0, json_js_2.sdIsString)(val.passphrase)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"passphrase\" of type \"JwtConfigAppSettingsAppAuth\"" });
    }
    const passphrase = val.passphrase;
    return { publicKeyId: publicKeyId, privateKey: privateKey, passphrase: passphrase };
}
...
function deserializeJwtConfigAppSettings(val) {
    if (!(0, json_js_3.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"JwtConfigAppSettings\"" });
    }
    if (val.clientID == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"clientID\" of type \"JwtConfigAppSettings\" to be defined" });
    }
    if (!(0, json_js_2.sdIsString)(val.clientID)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"clientID\" of type \"JwtConfigAppSettings\"" });
    }
    const clientId = val.clientID;
    if (val.clientSecret == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"clientSecret\" of type \"JwtConfigAppSettings\" to be defined" });
    }
    if (!(0, json_js_2.sdIsString)(val.clientSecret)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"clientSecret\" of type \"JwtConfigAppSettings\"" });
    }
    const clientSecret = val.clientSecret;
    if (val.appAuth == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"appAuth\" of type \"JwtConfigAppSettings\" to be defined" });
    }
    const appAuth = deserializeJwtConfigAppSettingsAppAuth(val.appAuth);
    return { clientId: clientId, clientSecret: clientSecret, appAuth: appAuth };
}
...
function deserializeJwtConfigFile(val) {
    ...
    if (!(val.enterpriseID == void 0) && !(0, json_js_2.sdIsString)(val.enterpriseID)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"enterpriseID\" of type \"JwtConfigFile\"" });
    }
    const enterpriseId = val.enterpriseID == void 0 ? void 0 : val.enterpriseID;
    if (!(val.userID == void 0) && !(0, json_js_2.sdIsString)(val.userID)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"userID\" of type \"JwtConfigFile\"" });
    }
    const userId = val.userID == void 0 ? void 0 : val.userID;
    if (val.boxAppSettings == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"boxAppSettings\" of type \"JwtConfigFile\" to be defined" });
    }
    const boxAppSettings = deserializeJwtConfigAppSettings(val.boxAppSettings);
    return { enterpriseId: enterpriseId, userId: userId, boxAppSettings: boxAppSettings };
}
