...
class TransferManager {
    ...
    transferOwnedFolder(userId_1, requestBody_1) {
        return __awaiter(this, arguments, void 0, function* (userId, requestBody, optionalsInput = {}) {
            const optionals = new TransferOwnedFolderOptionals({ queryParams: optionalsInput.queryParams, headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
            const queryParams = optionals.queryParams;
            const headers = optionals.headers;
            const cancellationToken = optionals.cancellationToken;
            const queryParamsMap = (0, utils_js_1.prepareParams)({ ["fields"]: queryParams.fields ? queryParams.fields.map(utils_js_2.toString).join(",") : undefined, ["notify"]: (0, utils_js_2.toString)(queryParams.notify) });
            const headersMap = (0, utils_js_1.prepareParams)(Object.assign({}, headers.extraHeaders));
            const response = yield this.networkSession.networkClient.fetch(new fetchOptions_generated_js_1.FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/users/", (0, utils_js_2.toString)(userId), "/folders/0"), method: "PUT", params: queryParamsMap, headers: headersMap, data: serializeTransferOwnedFolderRequestBody(requestBody), contentType: "application/json", responseFormat: "json", auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
...
function deserializeTransferOwnedFolderRequestBodyOwnedByField(val) {
    if (!(0, json_js_2.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"TransferOwnedFolderRequestBodyOwnedByField\"" });
    }
    if (val.id == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"id\" of type \"TransferOwnedFolderRequestBodyOwnedByField\" to be defined" });
    }
    if (!(0, json_js_1.sdIsString)(val.id)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting string for \"id\" of type \"TransferOwnedFolderRequestBodyOwnedByField\"" });
    }
    const id = val.id;
    return { id: id };
}
...
function deserializeTransferOwnedFolderRequestBody(val) {
    if (!(0, json_js_2.sdIsMap)(val)) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting a map for \"TransferOwnedFolderRequestBody\"" });
    }
    if (val.owned_by == void 0) {
        throw new errors_js_1.BoxSdkError({ message: "Expecting \"owned_by\" of type \"TransferOwnedFolderRequestBody\" to be defined" });
    }
