...
export function deserializeStoragePolicy(val: SerializedData): StoragePolicy {
    ...
    if (!(val.name == void 0) && !sdIsString(val.name)) {
        throw new BoxSdkError({ message: "Expecting string for \"name\" of type \"StoragePolicy\"" });
    }
    const name: undefined | string = val.name == void 0 ? void 0 : val.name;
    if (val.id == void 0) {
        throw new BoxSdkError({ message: "Expecting \"id\" of type \"StoragePolicy\" to be defined" });
    }
    if (!sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"StoragePolicy\"" });
    }
    const id: string = val.id;
    if (val.type == void 0) {
        throw new BoxSdkError({ message: "Expecting \"type\" of type \"StoragePolicy\" to be defined" });
    }
