...
export function serializeMetadataFieldFilterFloatRange(val: MetadataFieldFilterFloatRange): SerializedData {
    return { ["lt"]: val.lt, ["gt"]: val.gt };
}
export function deserializeMetadataFieldFilterFloatRange(val: SerializedData): MetadataFieldFilterFloatRange {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"MetadataFieldFilterFloatRange\"" });
    }
    if (!(val.lt == void 0) && !sdIsNumber(val.lt)) {
        throw new BoxSdkError({ message: "Expecting number for \"lt\" of type \"MetadataFieldFilterFloatRange\"" });
    }
    const lt: undefined | number = val.lt == void 0 ? void 0 : val.lt;
    if (!(val.gt == void 0) && !sdIsNumber(val.gt)) {
        throw new BoxSdkError({ message: "Expecting number for \"gt\" of type \"MetadataFieldFilterFloatRange\"" });
    }
