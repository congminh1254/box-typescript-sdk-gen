...
export function serializeAiAgentAskOrAiAgentReference(val: any): SerializedData {
...
export function deserializeAiAgentAskOrAiAgentReference(val: SerializedData): AiAgentAskOrAiAgentReference {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"AiAgentAskOrAiAgentReference\"" });
    }
    if (val.type == "ai_agent_ask") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize AiAgentAskOrAiAgentReference" });
}
