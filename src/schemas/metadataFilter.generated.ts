...
export function serializeMetadataFilterScopeField(val: MetadataFilterScopeField): SerializedData {
    return val;
}
export function deserializeMetadataFilterScopeField(val: SerializedData): MetadataFilterScopeField {
    if (val == "global") {
        return val;
    }
    if (val == "enterprise") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize MetadataFilterScopeField" });
}
export function serializeMetadataFilter(val: MetadataFilter): SerializedData {
    return { ["scope"]: val.scope == void 0 ? val.scope : serializeMetadataFilterScopeField(val.scope), ["templateKey"]: val.templateKey, ["filters"]: val.filters == void 0 ? val.filters : Object.fromEntries(Object.entries(val.filters).map(([k, v]: [
            string,
            any
        ]) => [k, serializeMetadataFieldFilterDateRangeOrMetadataFieldFilterFloatRangeOrArrayOfStringOrNumberOrString(v)])) as {
...
export function deserializeMetadataFilter(val: SerializedData): MetadataFilter {
    ...
    if (!(val.templateKey == void 0) && !sdIsString(val.templateKey)) {
        throw new BoxSdkError({ message: "Expecting string for \"templateKey\" of type \"MetadataFilter\"" });
    }
    ...
    if (!(val.filters == void 0) && !sdIsMap(val.filters)) {
        throw new BoxSdkError({ message: "Expecting object for \"filters\" of type \"MetadataFilter\"" });
    }
    const filters: undefined | {
        readonly [key: string]: MetadataFieldFilterDateRangeOrMetadataFieldFilterFloatRangeOrArrayOfStringOrNumberOrString;
    } = val.filters == void 0 ? void 0 : sdIsMap(val.filters) ? Object.fromEntries(Object.entries(val.filters).map(([k, v]: [
        string,
        any
    ]) => [k, deserializeMetadataFieldFilterDateRangeOrMetadataFieldFilterFloatRangeOrArrayOfStringOrNumberOrString(v)])) as {
        readonly [key: string]: any;
    } : {};
