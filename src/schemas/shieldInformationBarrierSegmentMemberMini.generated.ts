...
export function serializeShieldInformationBarrierSegmentMemberMini(val: ShieldInformationBarrierSegmentMemberMini): SerializedData {
    const base: any = serializeShieldInformationBarrierSegmentMemberBase(val);
    if (!sdIsMap(base)) {
        throw new BoxSdkError({ message: "Expecting a map for \"ShieldInformationBarrierSegmentMemberMini\"" });
    }
    return { ...base, ...{ ["user"]: val.user == void 0 ? val.user : serializeUserBase(val.user) } };
}
export function deserializeShieldInformationBarrierSegmentMemberMini(val: SerializedData): ShieldInformationBarrierSegmentMemberMini {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"ShieldInformationBarrierSegmentMemberMini\"" });
    }
    const user: undefined | UserBase = val.user == void 0 ? void 0 : deserializeUserBase(val.user);
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"ShieldInformationBarrierSegmentMemberMini\"" });
    }
