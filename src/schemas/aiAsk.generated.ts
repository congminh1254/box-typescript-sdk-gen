...
export function serializeAiAsk(val: AiAsk): SerializedData {
    return { ["mode"]: serializeAiAskModeField(val.mode), ["prompt"]: val.prompt, ["items"]: val.items.map(function (item: AiItemAsk): SerializedData {
        ...
        }) as readonly any[], ["include_citations"]: val.includeCitations, ["ai_agent"]: val.aiAgent == void 0 ? val.aiAgent : serializeAiAgentAskOrAiAgentReference(val.aiAgent) };
}
export function deserializeAiAsk(val: SerializedData): AiAsk {
    ...
    if (val.mode == void 0) {
        throw new BoxSdkError({ message: "Expecting \"mode\" of type \"AiAsk\" to be defined" });
    }
    const mode: AiAskModeField = deserializeAiAskModeField(val.mode);
    if (val.prompt == void 0) {
        throw new BoxSdkError({ message: "Expecting \"prompt\" of type \"AiAsk\" to be defined" });
    }
    if (!sdIsString(val.prompt)) {
        throw new BoxSdkError({ message: "Expecting string for \"prompt\" of type \"AiAsk\"" });
    }
    const prompt: string = val.prompt;
    if (val.items == void 0) {
        throw new BoxSdkError({ message: "Expecting \"items\" of type \"AiAsk\" to be defined" });
    }
    if (!sdIsList(val.items)) {
        throw new BoxSdkError({ message: "Expecting array for \"items\" of type \"AiAsk\"" });
    }
    const items: readonly AiItemAsk[] = sdIsList(val.items) ? val.items.map(function (itm: SerializedData): AiItemAsk {
        return deserializeAiItemAsk(itm);
    }) as readonly any[] : [];
    if (!(val.dialogue_history == void 0) && !sdIsList(val.dialogue_history)) {
        throw new BoxSdkError({ message: "Expecting array for \"dialogue_history\" of type \"AiAsk\"" });
    }
    ...
    if (!(val.include_citations == void 0) && !sdIsBoolean(val.include_citations)) {
        throw new BoxSdkError({ message: "Expecting boolean for \"include_citations\" of type \"AiAsk\"" });
    }
    const includeCitations: undefined | boolean = val.include_citations == void 0 ? void 0 : val.include_citations;
    const aiAgent: undefined | AiAgentAskOrAiAgentReference = val.ai_agent == void 0 ? void 0 : deserializeAiAgentAskOrAiAgentReference(val.ai_agent);
    return { mode: mode, prompt: prompt, items: items, dialogueHistory: dialogueHistory, includeCitations: includeCitations, aiAgent: aiAgent } satisfies AiAsk;
}
