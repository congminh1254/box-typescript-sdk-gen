...
export function serializeFileFullOrFolderMiniOrWebLink(val: any): SerializedData {
    if (val.type == "file") {
...
export function deserializeFileFullOrFolderMiniOrWebLink(val: SerializedData): FileFullOrFolderMiniOrWebLink {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"FileFullOrFolderMiniOrWebLink\"" });
    }
    if (val.type == "file") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize FileFullOrFolderMiniOrWebLink" });
}
