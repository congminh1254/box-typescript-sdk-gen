...
export function serializeLegalHoldPolicyAssignment(val: LegalHoldPolicyAssignment): SerializedData {
    const base: any = serializeLegalHoldPolicyAssignmentBase(val);
    if (!sdIsMap(base)) {
        throw new BoxSdkError({ message: "Expecting a map for \"LegalHoldPolicyAssignment\"" });
    }
    return { ...base, ...{ ["legal_hold_policy"]: val.legalHoldPolicy == void 0 ? val.legalHoldPolicy : serializeLegalHoldPolicyMini(val.legalHoldPolicy), ["assigned_to"]: val.assignedTo == void 0 ? val.assignedTo : serializeFileOrFolderOrWebLink(val.assignedTo), ["assigned_by"]: val.assignedBy == void 0 ? val.assignedBy : serializeUserMini(val.assignedBy), ["assigned_at"]: val.assignedAt == void 0 ? val.assignedAt : serializeDateTime(val.assignedAt), ["deleted_at"]: val.deletedAt == void 0 ? val.deletedAt : serializeDateTime(val.deletedAt) } };
}
export function deserializeLegalHoldPolicyAssignment(val: SerializedData): LegalHoldPolicyAssignment {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"LegalHoldPolicyAssignment\"" });
    }
    ...
    if (!(val.assigned_at == void 0) && !sdIsString(val.assigned_at)) {
        throw new BoxSdkError({ message: "Expecting string for \"assigned_at\" of type \"LegalHoldPolicyAssignment\"" });
    }
    ...
    if (!(val.deleted_at == void 0) && !sdIsString(val.deleted_at)) {
        throw new BoxSdkError({ message: "Expecting string for \"deleted_at\" of type \"LegalHoldPolicyAssignment\"" });
    }
    const deletedAt: undefined | DateTime = val.deleted_at == void 0 ? void 0 : deserializeDateTime(val.deleted_at);
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"LegalHoldPolicyAssignment\"" });
    }
