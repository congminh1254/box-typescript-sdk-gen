...
export function deserializeAiAgentAllowedEntity(val: SerializedData): AiAgentAllowedEntity {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"AiAgentAllowedEntity\"" });
    }
    if (val.type == "user") {
