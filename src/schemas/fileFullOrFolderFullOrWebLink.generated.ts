...
export function serializeFileFullOrFolderFullOrWebLink(val: any): SerializedData {
    if (val.type == "file") {
...
export function deserializeFileFullOrFolderFullOrWebLink(val: SerializedData): FileFullOrFolderFullOrWebLink {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"FileFullOrFolderFullOrWebLink\"" });
    }
    if (val.type == "file") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize FileFullOrFolderFullOrWebLink" });
}
