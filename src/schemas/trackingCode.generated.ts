...
export function serializeTrackingCodeTypeField(val: TrackingCodeTypeField): SerializedData {
    return val;
}
export function deserializeTrackingCodeTypeField(val: SerializedData): TrackingCodeTypeField {
...
export function serializeTrackingCode(val: TrackingCode): SerializedData {
    return { ["type"]: val.type == void 0 ? val.type : serializeTrackingCodeTypeField(val.type), ["name"]: val.name, ["value"]: val.value };
}
export function deserializeTrackingCode(val: SerializedData): TrackingCode {
    ...
    if (!(val.name == void 0) && !sdIsString(val.name)) {
        throw new BoxSdkError({ message: "Expecting string for \"name\" of type \"TrackingCode\"" });
    }
    const name: undefined | string = val.name == void 0 ? void 0 : val.name;
    if (!(val.value == void 0) && !sdIsString(val.value)) {
        throw new BoxSdkError({ message: "Expecting string for \"value\" of type \"TrackingCode\"" });
    }
