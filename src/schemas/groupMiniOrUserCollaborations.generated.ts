...
export function serializeGroupMiniOrUserCollaborations(val: any): SerializedData {
    if (val.type == "group") {
...
export function deserializeGroupMiniOrUserCollaborations(val: SerializedData): GroupMiniOrUserCollaborations {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"GroupMiniOrUserCollaborations\"" });
    }
    if (val.type == "group") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize GroupMiniOrUserCollaborations" });
}
