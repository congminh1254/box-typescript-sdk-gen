...
export function serializeCommentBaseTypeField(val: CommentBaseTypeField): SerializedData {
    return val;
}
export function deserializeCommentBaseTypeField(val: SerializedData): CommentBaseTypeField {
    if (val == "comment") {
        return val;
    }
...
export function serializeCommentBase(val: CommentBase): SerializedData {
    return { ["id"]: val.id, ["type"]: val.type == void 0 ? val.type : serializeCommentBaseTypeField(val.type) };
}
export function deserializeCommentBase(val: SerializedData): CommentBase {
    ...
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"CommentBase\"" });
    }
