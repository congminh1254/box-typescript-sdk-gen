...
export function serializeRetentionPolicyAssignments(val: RetentionPolicyAssignments): SerializedData {
    return { ["entries"]: val.entries == void 0 ? val.entries : val.entries.map(function (item: RetentionPolicyAssignment): SerializedData {
            return serializeRetentionPolicyAssignment(item);
        }) as readonly any[], ["limit"]: val.limit, ["next_marker"]: val.nextMarker };
}
export function deserializeRetentionPolicyAssignments(val: SerializedData): RetentionPolicyAssignments {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"RetentionPolicyAssignments\"" });
    }
    if (!(val.entries == void 0) && !sdIsList(val.entries)) {
        throw new BoxSdkError({ message: "Expecting array for \"entries\" of type \"RetentionPolicyAssignments\"" });
    }
    const entries: undefined | readonly RetentionPolicyAssignment[] = val.entries == void 0 ? void 0 : sdIsList(val.entries) ? val.entries.map(function (itm: SerializedData): RetentionPolicyAssignment {
        return deserializeRetentionPolicyAssignment(itm);
    }) as readonly any[] : [];
    if (!(val.limit == void 0) && !sdIsNumber(val.limit)) {
        throw new BoxSdkError({ message: "Expecting number for \"limit\" of type \"RetentionPolicyAssignments\"" });
    }
