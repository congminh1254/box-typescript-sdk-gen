...
export function serializeAiAgentReferenceOrAiAgentTextGen(val: any): SerializedData {
...
export function deserializeAiAgentReferenceOrAiAgentTextGen(val: SerializedData): AiAgentReferenceOrAiAgentTextGen {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"AiAgentReferenceOrAiAgentTextGen\"" });
    }
    if (val.type == "ai_agent_id") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize AiAgentReferenceOrAiAgentTextGen" });
}
