...
export function serializeEnterpriseBaseTypeField(val: EnterpriseBaseTypeField): SerializedData {
    return val;
}
export function deserializeEnterpriseBaseTypeField(val: SerializedData): EnterpriseBaseTypeField {
    if (val == "enterprise") {
        return val;
    }
    throw new BoxSdkError({ message: "Can't deserialize EnterpriseBaseTypeField" });
}
export function serializeEnterpriseBase(val: EnterpriseBase): SerializedData {
    return { ["id"]: val.id, ["type"]: val.type == void 0 ? val.type : serializeEnterpriseBaseTypeField(val.type) };
}
export function deserializeEnterpriseBase(val: SerializedData): EnterpriseBase {
    ...
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"EnterpriseBase\"" });
    }
