...
export function serializeEmailAliasTypeField(val: EmailAliasTypeField): SerializedData {
    return val;
}
export function deserializeEmailAliasTypeField(val: SerializedData): EmailAliasTypeField {
    if (val == "email_alias") {
        return val;
    }
...
export function serializeEmailAlias(val: EmailAlias): SerializedData {
    return { ["id"]: val.id, ["type"]: val.type == void 0 ? val.type : serializeEmailAliasTypeField(val.type), ["email"]: val.email, ["is_confirmed"]: val.isConfirmed };
}
export function deserializeEmailAlias(val: SerializedData): EmailAlias {
    ...
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"EmailAlias\"" });
    }
    ...
    if (!(val.email == void 0) && !sdIsString(val.email)) {
        throw new BoxSdkError({ message: "Expecting string for \"email\" of type \"EmailAlias\"" });
    }
