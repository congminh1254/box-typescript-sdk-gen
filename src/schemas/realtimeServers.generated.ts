...
export function serializeRealtimeServers(val: RealtimeServers): SerializedData {
    return { ["chunk_size"]: val.chunkSize, ["entries"]: val.entries == void 0 ? val.entries : val.entries.map(function (item: RealtimeServer): SerializedData {
            return serializeRealtimeServer(item);
        }) as readonly any[] };
}
export function deserializeRealtimeServers(val: SerializedData): RealtimeServers {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"RealtimeServers\"" });
    }
    if (!(val.chunk_size == void 0) && !sdIsNumber(val.chunk_size)) {
        throw new BoxSdkError({ message: "Expecting number for \"chunk_size\" of type \"RealtimeServers\"" });
    }
    const chunkSize: undefined | number = val.chunk_size == void 0 ? void 0 : val.chunk_size;
    if (!(val.entries == void 0) && !sdIsList(val.entries)) {
        throw new BoxSdkError({ message: "Expecting array for \"entries\" of type \"RealtimeServers\"" });
    }
    const entries: undefined | readonly RealtimeServer[] = val.entries == void 0 ? void 0 : sdIsList(val.entries) ? val.entries.map(function (itm: SerializedData): RealtimeServer {
