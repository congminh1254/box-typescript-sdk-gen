...
export function serializeFileVersionRetentionTypeField(val: FileVersionRetentionTypeField): SerializedData {
    return val;
}
export function deserializeFileVersionRetentionTypeField(val: SerializedData): FileVersionRetentionTypeField {
    if (val == "file_version_retention") {
        return val;
    }
    throw new BoxSdkError({ message: "Can't deserialize FileVersionRetentionTypeField" });
}
export function serializeFileVersionRetention(val: FileVersionRetention): SerializedData {
    return { ["id"]: val.id, ["type"]: val.type == void 0 ? val.type : serializeFileVersionRetentionTypeField(val.type), ["file_version"]: val.fileVersion == void 0 ? val.fileVersion : serializeFileVersionMini(val.fileVersion), ["file"]: val.file == void 0 ? val.file : serializeFileMini(val.file), ["applied_at"]: val.appliedAt == void 0 ? val.appliedAt : serializeDateTime(val.appliedAt), ["disposition_at"]: val.dispositionAt == void 0 ? val.dispositionAt : serializeDateTime(val.dispositionAt), ["winning_retention_policy"]: val.winningRetentionPolicy == void 0 ? val.winningRetentionPolicy : serializeRetentionPolicyMini(val.winningRetentionPolicy) };
}
export function deserializeFileVersionRetention(val: SerializedData): FileVersionRetention {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"FileVersionRetention\"" });
    }
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"FileVersionRetention\"" });
    }
    ...
    if (!(val.applied_at == void 0) && !sdIsString(val.applied_at)) {
        throw new BoxSdkError({ message: "Expecting string for \"applied_at\" of type \"FileVersionRetention\"" });
    }
    const appliedAt: undefined | DateTime = val.applied_at == void 0 ? void 0 : deserializeDateTime(val.applied_at);
    if (!(val.disposition_at == void 0) && !sdIsString(val.disposition_at)) {
        throw new BoxSdkError({ message: "Expecting string for \"disposition_at\" of type \"FileVersionRetention\"" });
    }
