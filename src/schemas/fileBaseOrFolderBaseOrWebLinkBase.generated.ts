...
export function serializeFileBaseOrFolderBaseOrWebLinkBase(val: any): SerializedData {
    if (val.type == "file") {
...
export function deserializeFileBaseOrFolderBaseOrWebLinkBase(val: SerializedData): FileBaseOrFolderBaseOrWebLinkBase {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"FileBaseOrFolderBaseOrWebLinkBase\"" });
    }
    if (val.type == "file") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize FileBaseOrFolderBaseOrWebLinkBase" });
}
