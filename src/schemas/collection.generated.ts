...
export function serializeCollectionTypeField(val: CollectionTypeField): SerializedData {
    return val;
}
export function deserializeCollectionTypeField(val: SerializedData): CollectionTypeField {
    if (val == "collection") {
        return val;
    }
    throw new BoxSdkError({ message: "Can't deserialize CollectionTypeField" });
}
export function serializeCollectionNameField(val: CollectionNameField): SerializedData {
    return val;
}
export function deserializeCollectionNameField(val: SerializedData): CollectionNameField {
    if (val == "Favorites") {
        return val;
    }
    throw new BoxSdkError({ message: "Can't deserialize CollectionNameField" });
}
export function serializeCollectionCollectionTypeField(val: CollectionCollectionTypeField): SerializedData {
    return val;
}
export function deserializeCollectionCollectionTypeField(val: SerializedData): CollectionCollectionTypeField {
    if (val == "favorites") {
        return val;
    }
    throw new BoxSdkError({ message: "Can't deserialize CollectionCollectionTypeField" });
}
export function serializeCollection(val: Collection): SerializedData {
    return { ["id"]: val.id, ["type"]: val.type == void 0 ? val.type : serializeCollectionTypeField(val.type), ["name"]: val.name == void 0 ? val.name : serializeCollectionNameField(val.name), ["collection_type"]: val.collectionType == void 0 ? val.collectionType : serializeCollectionCollectionTypeField(val.collectionType) };
}
export function deserializeCollection(val: SerializedData): Collection {
    ...
    if (!(val.id == void 0) && !sdIsString(val.id)) {
        throw new BoxSdkError({ message: "Expecting string for \"id\" of type \"Collection\"" });
    }
