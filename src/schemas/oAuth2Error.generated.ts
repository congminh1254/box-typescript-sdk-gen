...
export function deserializeOAuth2Error(val: SerializedData): OAuth2Error {
    ...
    if (!(val.error == void 0) && !sdIsString(val.error)) {
        throw new BoxSdkError({ message: "Expecting string for \"error\" of type \"OAuth2Error\"" });
    }
    const error: undefined | string = val.error == void 0 ? void 0 : val.error;
    if (!(val.error_description == void 0) && !sdIsString(val.error_description)) {
        throw new BoxSdkError({ message: "Expecting string for \"error_description\" of type \"OAuth2Error\"" });
    }
