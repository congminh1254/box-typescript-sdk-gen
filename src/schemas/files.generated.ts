...
export function serializeFiles(val: Files): SerializedData {
    return { ["total_count"]: val.totalCount, ["entries"]: val.entries == void 0 ? val.entries : val.entries.map(function (item: FileFull): SerializedData {
...
export function deserializeFiles(val: SerializedData): Files {
    ...
    if (!(val.total_count == void 0) && !sdIsNumber(val.total_count)) {
        throw new BoxSdkError({ message: "Expecting number for \"total_count\" of type \"Files\"" });
    }
    const totalCount: undefined | number = val.total_count == void 0 ? void 0 : val.total_count;
    if (!(val.entries == void 0) && !sdIsList(val.entries)) {
        throw new BoxSdkError({ message: "Expecting array for \"entries\" of type \"Files\"" });
    }
    const entries: undefined | readonly FileFull[] = val.entries == void 0 ? void 0 : sdIsList(val.entries) ? val.entries.map(function (itm: SerializedData): FileFull {
