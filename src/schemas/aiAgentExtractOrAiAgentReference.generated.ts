...
export function serializeAiAgentExtractOrAiAgentReference(val: any): SerializedData {
...
export function deserializeAiAgentExtractOrAiAgentReference(val: SerializedData): AiAgentExtractOrAiAgentReference {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"AiAgentExtractOrAiAgentReference\"" });
    }
    if (val.type == "ai_agent_extract") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize AiAgentExtractOrAiAgentReference" });
}
