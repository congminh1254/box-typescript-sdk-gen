...
export function serializeFolderLocks(val: FolderLocks): SerializedData {
    return { ["entries"]: val.entries == void 0 ? val.entries : val.entries.map(function (item: FolderLock): SerializedData {
...
export function deserializeFolderLocks(val: SerializedData): FolderLocks {
    ...
    if (!(val.entries == void 0) && !sdIsList(val.entries)) {
        throw new BoxSdkError({ message: "Expecting array for \"entries\" of type \"FolderLocks\"" });
    }
    const entries: undefined | readonly FolderLock[] = val.entries == void 0 ? void 0 : sdIsList(val.entries) ? val.entries.map(function (itm: SerializedData): FolderLock {
        return deserializeFolderLock(itm);
    }) as readonly any[] : [];
    if (!(val.limit == void 0) && !sdIsString(val.limit)) {
        throw new BoxSdkError({ message: "Expecting string for \"limit\" of type \"FolderLocks\"" });
    }
    ...
    return { entries: entries, limit: limit, nextMarker: nextMarker } satisfies FolderLocks;
}
