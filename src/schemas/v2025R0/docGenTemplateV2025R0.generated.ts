...
export function serializeDocGenTemplateV2025R0(val: DocGenTemplateV2025R0): SerializedData {
    const base: any = serializeDocGenTemplateBaseV2025R0(val);
    if (!sdIsMap(base)) {
        throw new BoxSdkError({ message: "Expecting a map for \"DocGenTemplateV2025R0\"" });
    }
    return { ...base, ...{ ["file_name"]: val.fileName } };
}
export function deserializeDocGenTemplateV2025R0(val: SerializedData): DocGenTemplateV2025R0 {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"DocGenTemplateV2025R0\"" });
    }
    if (!(val.file_name == void 0) && !sdIsString(val.file_name)) {
        throw new BoxSdkError({ message: "Expecting string for \"file_name\" of type \"DocGenTemplateV2025R0\"" });
    }
