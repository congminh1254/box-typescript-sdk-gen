...
export function serializeShieldInformationBarrierReference(val: ShieldInformationBarrierReference): SerializedData {
    return { ["shield_information_barrier"]: val.shieldInformationBarrier == void 0 ? val.shieldInformationBarrier : serializeShieldInformationBarrierBase(val.shieldInformationBarrier) };
}
export function deserializeShieldInformationBarrierReference(val: SerializedData): ShieldInformationBarrierReference {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"ShieldInformationBarrierReference\"" });
    }
