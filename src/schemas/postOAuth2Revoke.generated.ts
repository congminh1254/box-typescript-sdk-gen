...
export function serializePostOAuth2Revoke(val: PostOAuth2Revoke): SerializedData {
    return { ["client_id"]: val.clientId, ["client_secret"]: val.clientSecret, ["token"]: val.token };
}
export function deserializePostOAuth2Revoke(val: SerializedData): PostOAuth2Revoke {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"PostOAuth2Revoke\"" });
    }
    if (!(val.client_id == void 0) && !sdIsString(val.client_id)) {
        throw new BoxSdkError({ message: "Expecting string for \"client_id\" of type \"PostOAuth2Revoke\"" });
    }
    ...
    if (!(val.client_secret == void 0) && !sdIsString(val.client_secret)) {
        throw new BoxSdkError({ message: "Expecting string for \"client_secret\" of type \"PostOAuth2Revoke\"" });
    }
    ...
    if (!(val.token == void 0) && !sdIsString(val.token)) {
        throw new BoxSdkError({ message: "Expecting string for \"token\" of type \"PostOAuth2Revoke\"" });
    }
