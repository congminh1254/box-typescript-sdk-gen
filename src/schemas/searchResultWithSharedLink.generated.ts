...
export function serializeSearchResultWithSharedLink(val: SearchResultWithSharedLink): SerializedData {
    return { ["accessible_via_shared_link"]: val.accessibleViaSharedLink, ["item"]: val.item == void 0 ? val.item : serializeFileFullOrFolderFullOrWebLink(val.item), ["type"]: val.type };
}
export function deserializeSearchResultWithSharedLink(val: SerializedData): SearchResultWithSharedLink {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"SearchResultWithSharedLink\"" });
    }
    if (!(val.accessible_via_shared_link == void 0) && !sdIsString(val.accessible_via_shared_link)) {
        throw new BoxSdkError({ message: "Expecting string for \"accessible_via_shared_link\" of type \"SearchResultWithSharedLink\"" });
    }
    ...
    if (!(val.type == void 0) && !sdIsString(val.type)) {
        throw new BoxSdkError({ message: "Expecting string for \"type\" of type \"SearchResultWithSharedLink\"" });
    }
