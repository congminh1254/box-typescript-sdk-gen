...
export function serializeAiExtract(val: AiExtract): SerializedData {
    return { ["prompt"]: val.prompt, ["items"]: val.items.map(function (item: AiItemBase): SerializedData {
            return serializeAiItemBase(item);
        }) as readonly any[], ["ai_agent"]: val.aiAgent == void 0 ? val.aiAgent : serializeAiAgentExtractOrAiAgentReference(val.aiAgent) };
}
export function deserializeAiExtract(val: SerializedData): AiExtract {
    ...
    if (val.prompt == void 0) {
        throw new BoxSdkError({ message: "Expecting \"prompt\" of type \"AiExtract\" to be defined" });
    }
    if (!sdIsString(val.prompt)) {
        throw new BoxSdkError({ message: "Expecting string for \"prompt\" of type \"AiExtract\"" });
    }
    const prompt: string = val.prompt;
    if (val.items == void 0) {
        throw new BoxSdkError({ message: "Expecting \"items\" of type \"AiExtract\" to be defined" });
    }
    if (!sdIsList(val.items)) {
        throw new BoxSdkError({ message: "Expecting array for \"items\" of type \"AiExtract\"" });
    }
    ...
    const aiAgent: undefined | AiAgentExtractOrAiAgentReference = val.ai_agent == void 0 ? void 0 : deserializeAiAgentExtractOrAiAgentReference(val.ai_agent);
