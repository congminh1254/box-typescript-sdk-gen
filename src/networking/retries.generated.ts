...
export class BoxRetryStrategy implements RetryStrategy {
     ...
     * @returns {Promise<boolean>}
     */
    async shouldRetry(fetchOptions: FetchOptions, fetchResponse: FetchResponse, attemptNumber: number): Promise<boolean> {
        const isSuccessful: boolean = fetchResponse.status >= 200 && fetchResponse.status < 400;
        const retryAfterHeader: string = fetchResponse.headers["Retry-After"];
        const isAcceptedWithRetryAfter: boolean = fetchResponse.status == 202 && !(retryAfterHeader == void 0);
    ...
    retryAfter(fetchOptions: FetchOptions, fetchResponse: FetchResponse, attemptNumber: number): number {
        ...
        const randomization: number = random(1 - this.retryRandomizationFactor, 1 + this.retryRandomizationFactor);
