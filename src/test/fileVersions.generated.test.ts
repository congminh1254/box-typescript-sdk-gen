...
test("testCreateListGetPromoteFileVersion", async function testCreateListGetPromoteFileVersion(): Promise<any> {
    ...
    const files: Files = await client.uploads.uploadFile({ attributes: { name: oldName, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(10) } satisfies UploadFileRequestBody);
    ...
    const newFiles: Files = await client.uploads.uploadFileVersion(file.id, { attributes: { name: newName } satisfies UploadFileVersionRequestBodyAttributesField, file: generateByteStream(20) } satisfies UploadFileVersionRequestBody);
    ...
    const fileVersions: FileVersions = await client.fileVersions.getFileVersions(file.id);
    if (!(fileVersions.totalCount == 1)) {
        throw new Error("Assertion failed");
    }
    const fileVersion: FileVersionFull = await client.fileVersions.getFileVersionById(file.id, fileVersions.entries![0].id);
    ...
    const fileWithPromotedVersion: FileFull = await client.files.getFileById(file.id);
...
test("testRemoveAndRestoreFileVersion", async function testRemoveAndRestoreFileVersion(): Promise<any> {
    ...
    const files: Files = await client.uploads.uploadFile({ attributes: { name: oldName, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(10) } satisfies UploadFileRequestBody);
    const file: FileFull = files.entries![0];
    await client.uploads.uploadFileVersion(file.id, { attributes: { name: newName } satisfies UploadFileVersionRequestBodyAttributesField, file: generateByteStream(20) } satisfies UploadFileVersionRequestBody)
    const fileVersions: FileVersions = await client.fileVersions.getFileVersions(file.id);
    if (!(fileVersions.totalCount == 1)) {
        throw new Error("Assertion failed");
    }
    const fileVersion: FileVersionFull = await client.fileVersions.getFileVersionById(file.id, fileVersions.entries![0].id, { queryParams: { fields: ["trashed_at" as string, "trashed_by" as string, "restored_at" as string, "restored_by" as string] } satisfies GetFileVersionByIdQueryParams } satisfies GetFileVersionByIdOptionalsInput);
    ...
    await client.fileVersions.deleteFileVersionById(file.id, fileVersion.id)
    const deletedFileVersion: FileVersionFull = await client.fileVersions.getFileVersionById(file.id, fileVersions.entries![0].id, { queryParams: { fields: ["trashed_at" as string, "trashed_by" as string, "restored_at" as string, "restored_by" as string] } satisfies GetFileVersionByIdQueryParams } satisfies GetFileVersionByIdOptionalsInput);
    ...
    const restoredFileVersion: FileVersionFull = await client.fileVersions.getFileVersionById(file.id, fileVersions.entries![0].id, { queryParams: { fields: ["trashed_at" as string, "trashed_by" as string, "restored_at" as string, "restored_by" as string] } satisfies GetFileVersionByIdQueryParams } satisfies GetFileVersionByIdOptionalsInput);
