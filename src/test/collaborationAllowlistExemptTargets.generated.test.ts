...
test("testCollaborationAllowlistExemptTargets", async function testCollaborationAllowlistExemptTargets(): Promise<any> {
    const exemptTargets: CollaborationAllowlistExemptTargets = await client.collaborationAllowlistExemptTargets.getCollaborationWhitelistExemptTargets();
    ...
    const newExemptTarget: CollaborationAllowlistExemptTarget = await client.collaborationAllowlistExemptTargets.createCollaborationWhitelistExemptTarget({ user: { id: user.id } satisfies CreateCollaborationWhitelistExemptTargetRequestBodyUserField } satisfies CreateCollaborationWhitelistExemptTargetRequestBody);
    if (!(toString(newExemptTarget.type) as string == "collaboration_whitelist_exempt_target")) {
        throw new Error("Assertion failed");
    }
    if (!(newExemptTarget.user!.id == user.id)) {
        throw new Error("Assertion failed");
    }
    const exemptTarget: CollaborationAllowlistExemptTarget = await client.collaborationAllowlistExemptTargets.getCollaborationWhitelistExemptTargetById(newExemptTarget.id!);
    ...
    await expect(async () => { await client.collaborationAllowlistExemptTargets.getCollaborationWhitelistExemptTargetById(exemptTarget.id!); }).rejects.toThrow();
