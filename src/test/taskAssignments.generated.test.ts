...
test("testCreateUpdateGetDeleteTaskAssignment", async function testCreateUpdateGetDeleteTaskAssignment(): Promise<any> {
    ...
    const taskAssignment: TaskAssignment = await client.taskAssignments.createTaskAssignment({ task: new CreateTaskAssignmentRequestBodyTaskField({ type: "task" as CreateTaskAssignmentRequestBodyTaskTypeField, id: task.id! }), assignTo: { id: currentUser.id } satisfies CreateTaskAssignmentRequestBodyAssignToField } satisfies CreateTaskAssignmentRequestBody);
    ...
    const taskAssignmentById: TaskAssignment = await client.taskAssignments.getTaskAssignmentById(taskAssignment.id!);
    if (!(taskAssignmentById.id == taskAssignment.id)) {
        throw new Error("Assertion failed");
    }
    const taskAssignmentsOnTask: TaskAssignments = await client.taskAssignments.getTaskAssignments(task.id!);
    if (!(taskAssignmentsOnTask.totalCount! == 1)) {
        throw new Error("Assertion failed");
    }
    const updatedTaskAssignment: TaskAssignment = await client.taskAssignments.updateTaskAssignmentById(taskAssignment.id!, { requestBody: { message: "updated message", resolutionState: "approved" as UpdateTaskAssignmentByIdRequestBodyResolutionStateField } satisfies UpdateTaskAssignmentByIdRequestBody } satisfies UpdateTaskAssignmentByIdOptionalsInput);
    if (!(updatedTaskAssignment.message == "updated message")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(updatedTaskAssignment.resolutionState) as string == "approved")) {
        throw new Error("Assertion failed");
    }
    await expect(async () => { await client.taskAssignments.deleteTaskAssignmentById(taskAssignment.id!); }).rejects.toThrow();
