...
test("testGetFileVersionLegalHolds", async function testGetFileVersionLegalHolds(): Promise<any> {
    ...
    const fileVersionLegalHolds: FileVersionLegalHolds = await client.fileVersionLegalHolds.getFileVersionLegalHolds({ policyId: policyId } satisfies GetFileVersionLegalHoldsQueryParams);
    const fileVersionLegalHoldsCount: number = fileVersionLegalHolds.entries!.length;
...
test("testGetFileVersionLegalHoldById", async function testGetFileVersionLegalHoldById(): Promise<any> {
    ...
    await expect(async () => { await client.fileVersionLegalHolds.getFileVersionLegalHoldById(fileVersionLegalHoldId); }).rejects.toThrow();
});
export {};
