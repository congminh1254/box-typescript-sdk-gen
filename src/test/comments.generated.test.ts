...
test("testComments", async function testComments(): Promise<any> {
    ...
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: fileName, parent: { id: parentId } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: fileByteStream } satisfies UploadFileRequestBody);
    ...
    const newComment: CommentFull = await client.comments.createComment({ message: message, item: { id: fileId, type: "file" as CreateCommentRequestBodyItemTypeField } satisfies CreateCommentRequestBodyItemField } satisfies CreateCommentRequestBody);
    ...
    const newReplyComment: CommentFull = await client.comments.createComment({ message: message, item: { id: newComment.id!, type: "comment" as CreateCommentRequestBodyItemTypeField } satisfies CreateCommentRequestBodyItemField } satisfies CreateCommentRequestBody);
    ...
    const receivedComment: CommentFull = await client.comments.getCommentById(newComment.id!);
    if (!(receivedComment.message! == newComment.message!)) {
        throw new Error("Assertion failed");
    }
    await client.comments.deleteCommentById(newComment.id!)
    await expect(async () => { await client.comments.getCommentById(newComment.id!); }).rejects.toThrow();
