...
test("testCreateUpdateGetDeleteRetentionPolicyAssignment", async function testCreateUpdateGetDeleteRetentionPolicyAssignment(): Promise<any> {
    ...
    const newVersions: Files = await client.uploads.uploadFileVersion(file.id, { attributes: { name: getUuid() } satisfies UploadFileVersionRequestBodyAttributesField, file: generateByteStream(20) } satisfies UploadFileVersionRequestBody);
    ...
    const retentionPolicyAssignmentById: RetentionPolicyAssignment = await client.retentionPolicyAssignments.getRetentionPolicyAssignmentById(retentionPolicyAssignment.id);
    if (!(retentionPolicyAssignmentById.id == retentionPolicyAssignment.id)) {
        throw new Error("Assertion failed");
    }
    const retentionPolicyAssignments: RetentionPolicyAssignments = await client.retentionPolicyAssignments.getRetentionPolicyAssignments(retentionPolicy.id);
    if (!(retentionPolicyAssignments.entries!.length == 1)) {
        throw new Error("Assertion failed");
    }
    const filesUnderRetention: FilesUnderRetention = await client.retentionPolicyAssignments.getFilesUnderRetentionPolicyAssignment(retentionPolicyAssignment.id);
    ...
    const retentionPolicyAssignmentsAfterDelete: RetentionPolicyAssignments = await client.retentionPolicyAssignments.getRetentionPolicyAssignments(retentionPolicy.id);
