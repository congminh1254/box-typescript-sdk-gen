...
test("testShieldInformationBarrierReports", async function testShieldInformationBarrierReports(): Promise<any> {
    ...
    const existingReports: ShieldInformationBarrierReports = await client.shieldInformationBarrierReports.getShieldInformationBarrierReports({ shieldInformationBarrierId: barrierId } satisfies GetShieldInformationBarrierReportsQueryParams);
    if (existingReports.entries!.length > 0) {
        return void 0;
    }
    const createdReport: ShieldInformationBarrierReport = await client.shieldInformationBarrierReports.createShieldInformationBarrierReport({ shieldInformationBarrier: { id: barrierId, type: "shield_information_barrier" as ShieldInformationBarrierBaseTypeField } satisfies ShieldInformationBarrierBase } satisfies ShieldInformationBarrierReference);
    if (!(createdReport.shieldInformationBarrier!.shieldInformationBarrier!.id == barrierId)) {
        throw new Error("Assertion failed");
    }
    const retrievedReport: ShieldInformationBarrierReport = await client.shieldInformationBarrierReports.getShieldInformationBarrierReportById(createdReport.id!);
    if (!(retrievedReport.id == createdReport.id)) {
        throw new Error("Assertion failed");
    }
    const retrievedReports: ShieldInformationBarrierReports = await client.shieldInformationBarrierReports.getShieldInformationBarrierReports({ shieldInformationBarrierId: barrierId } satisfies GetShieldInformationBarrierReportsQueryParams);
