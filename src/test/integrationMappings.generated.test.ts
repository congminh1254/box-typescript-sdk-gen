...
test("testSlackIntegrationMappings", async function testSlackIntegrationMappings(): Promise<any> {
    ...
    await expect(async () => { await userClient.integrationMappings.updateSlackIntegrationMappingById(integrationMappingId, { requestBody: { boxItem: new IntegrationMappingBoxItemSlack({ id: "1234567" }) } satisfies UpdateSlackIntegrationMappingByIdRequestBody } satisfies UpdateSlackIntegrationMappingByIdOptionalsInput); }).rejects.toThrow();
    await expect(async () => { await userClient.integrationMappings.deleteSlackIntegrationMappingById(integrationMappingId); }).rejects.toThrow();
...
test("testTeamsIntegrationMappings", async function testTeamsIntegrationMappings(): Promise<any> {
    ...
    const userId: string = getEnvVar("USER_ID");
    const userClient: BoxClient = getDefaultClientWithUserSubject(userId);
    await expect(async () => { await userClient.integrationMappings.createTeamsIntegrationMapping({ partnerItem: { type: "channel" as IntegrationMappingPartnerItemTeamsCreateRequestTypeField, id: partnerItemId, tenantId: tenantId, teamId: teamId } satisfies IntegrationMappingPartnerItemTeamsCreateRequest, boxItem: new FolderReference({ id: folder.id }) } satisfies IntegrationMappingTeamsCreateRequest); }).rejects.toThrow();
    await expect(async () => { await userClient.integrationMappings.getTeamsIntegrationMapping(); }).rejects.toThrow();
    const integrationMappingId: string = "123456";
    await expect(async () => { await userClient.integrationMappings.updateTeamsIntegrationMappingById(integrationMappingId, { requestBody: { boxItem: new FolderReference({ id: "1234567" }) } satisfies UpdateTeamsIntegrationMappingByIdRequestBody } satisfies UpdateTeamsIntegrationMappingByIdOptionalsInput); }).rejects.toThrow();
    await expect(async () => { await userClient.integrationMappings.deleteTeamsIntegrationMappingById(integrationMappingId); }).rejects.toThrow();
    await client.folders.deleteFolderById(folder.id)
});
export {};
