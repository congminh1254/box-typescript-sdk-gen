...
test("testIntegrationMappings", async function testIntegrationMappings(): Promise<any> {
    ...
    await expect(async () => { await userClient.integrationMappings.createSlackIntegrationMapping({ partnerItem: new IntegrationMappingPartnerItemSlack({ id: partnerItemId, slackOrgId: slackOrgId }), boxItem: new IntegrationMappingBoxItemSlack({ id: folder.id }) } satisfies IntegrationMappingSlackCreateRequest); }).rejects.toThrow();
    const integrationMappings: IntegrationMappings = await userClient.integrationMappings.getSlackIntegrationMapping();
