...
test("testTrashedWebLinks", async function testTrashedWebLinks(): Promise<any> {
    ...
    const weblink: WebLink = await client.webLinks.createWebLink({ url: url, parent: { id: parent.id } satisfies CreateWebLinkRequestBodyParentField, name: name, description: description } satisfies CreateWebLinkRequestBody);
    await client.webLinks.deleteWebLinkById(weblink.id)
    const fromTrash: TrashWebLink = await client.trashedWebLinks.getTrashedWebLinkById(weblink.id);
    ...
    const fromApiAfterTrashed: WebLink = await client.webLinks.getWebLinkById(weblink.id);
    if (!(toString(fromApiAfterTrashed.itemStatus) as string == "trashed")) {
        throw new Error("Assertion failed");
    }
    const restoredWeblink: TrashWebLinkRestored = await client.trashedWebLinks.restoreWeblinkFromTrash(weblink.id);
    ...
    await expect(async () => { await client.trashedWebLinks.getTrashedWebLinkById(weblink.id); }).rejects.toThrow();
});
export {};
