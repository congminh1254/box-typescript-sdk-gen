...
test("testListCollaborations", async function testListCollaborations(): Promise<any> {
    ...
    const fileCollaborations: Collaborations = await client.listCollaborations.getFileCollaborations(file.id);
    ...
    const pendingCollaborations: CollaborationsOffsetPaginated = await client.listCollaborations.getCollaborations({ status: "pending" as GetCollaborationsQueryParamsStatusField } satisfies GetCollaborationsQueryParams);
    if (!(pendingCollaborations.entries!.length >= 0)) {
        throw new Error("Assertion failed");
    }
    const groupCollaborations: CollaborationsOffsetPaginated = await client.listCollaborations.getGroupCollaborations(group.id);
