...
test("testCreateMetaDataQueryExecuteRead", async function testCreateMetaDataQueryExecuteRead(): Promise<any> {
    ...
    const metadata: MetadataFull = await client.fileMetadata.createFileMetadataById(file.id, "enterprise" as CreateFileMetadataByIdScope, templateKey, { ["name"]: "John", ["age"]: 23, ["birthDate"]: "2001-01-03T02:20:50.520Z", ["countryCode"]: "US", ["sports"]: ["basketball", "tennis"] });
    ...
    const searchFrom: string = "".concat(template.scope!, ".", template.templateKey) as string;
    const query: MetadataQueryResults = await client.search.searchByMetadataQuery({ ancestorFolderId: "0", from: searchFrom, query: "name = :name AND age < :age AND birthDate >= :birthDate AND countryCode = :countryCode AND sports = :sports", queryParams: { ["name"]: "John", ["age"]: 50, ["birthDate"]: "2001-01-01T02:20:10.120Z", ["countryCode"]: "US", ["sports"]: ["basketball", "tennis"] } } satisfies MetadataQuery);
...
test("testMetadataFilters", async function testMetadataFilters(): Promise<any> {
    ...
    const files: Files = await client.uploads.uploadFile({ attributes: { name: getUuid(), parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(10) } satisfies UploadFileRequestBody);
    const file: FileFull = files.entries![0];
    const metadata: MetadataFull = await client.fileMetadata.createFileMetadataById(file.id, "enterprise" as CreateFileMetadataByIdScope, templateKey, { ["floatField"]: 10, ["stringField"]: "stringValue", ["dateField"]: "2035-01-02T00:00:00Z", ["enumField"]: "enumValue2", ["multiSelectField"]: ["multiSelectValue1", "multiSelectValue2"] });
    const query: SearchResultsOrSearchResultsWithSharedLinks = await client.search.searchForContent({ ancestorFolderIds: ["0" as string], mdfilters: [{ filters: { ["stringField"]: "stringValue", ["dateField"]: { lt: dateTimeFromString("2035-01-01T00:00:00Z"), gt: dateTimeFromString("2035-01-03T00:00:00Z") } satisfies MetadataFieldFilterDateRange, ["floatField"]: { lt: 9.5, gt: 10.5 } satisfies MetadataFieldFilterFloatRange, ["enumField"]: "enumValue2", ["multiSelectField"]: ["multiSelectValue1", "multiSelectValue2"] }, scope: "enterprise" as MetadataFilterScopeField, templateKey: templateKey } satisfies MetadataFilter] } satisfies SearchForContentQueryParams);
    if (!(query.entries!.length >= 0)) {
        throw new Error("Assertion failed");
    }
    await client.metadataTemplates.deleteMetadataTemplate("enterprise" as DeleteMetadataTemplateScope, template.templateKey!)
...
test("testGetSearch", async function testGetSearch(): Promise<any> {
    ...
    const search: SearchResultsOrSearchResultsWithSharedLinks = await client.search.searchForContent({ ancestorFolderIds: ["0" as string], query: keyword, trashContent: "non_trashed_only" as SearchForContentQueryParamsTrashContentField } satisfies SearchForContentQueryParams);
    ...
    const searchWithSharedLink: SearchResultsOrSearchResultsWithSharedLinks = await client.search.searchForContent({ ancestorFolderIds: ["0" as string], query: keyword, trashContent: "non_trashed_only" as SearchForContentQueryParamsTrashContentField, includeRecentSharedLinks: true } satisfies SearchForContentQueryParams);
    if (!(searchWithSharedLink.entries!.length >= 0)) {
        throw new Error("Assertion failed");
    }
    if (!(toString(searchWithSharedLink.type) as string == "search_results_with_shared_links")) {
        throw new Error("Assertion failed");
    }
});
export {};
