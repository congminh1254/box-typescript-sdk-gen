...
test("testCreateGetCancelAndListSignRequest", async function testCreateGetCancelAndListSignRequest(): Promise<any> {
    ...
    if (!(toString(createdSignRequest.signers![1].role) as string == "signer")) {
        throw new Error("Assertion failed");
    }
    if (!(createdSignRequest.parentFolder!.id == destinationFolder.id)) {
        throw new Error("Assertion failed");
    }
    if (!(dateToString(createdSignRequest.prefillTags![0].dateValue!) == "2035-01-01")) {
        throw new Error("Assertion failed");
    }
    const newSignRequest: SignRequest = await client.signRequests.getSignRequestById(createdSignRequest.id!);
    ...
    const cancelledSignRequest: SignRequest = await client.signRequests.cancelSignRequest(createdSignRequest.id!);
    if (!(toString(cancelledSignRequest.status) as string == "cancelled")) {
        throw new Error("Assertion failed");
    }
    const signRequests: SignRequests = await client.signRequests.getSignRequests();
    if (!(toString(signRequests.entries![0].type!) as string == "sign-request")) {
        throw new Error("Assertion failed");
    }
    await client.folders.deleteFolderById(destinationFolder.id, { queryParams: { recursive: true } satisfies DeleteFolderByIdQueryParams } satisfies DeleteFolderByIdOptionalsInput)
...
test("testCreateSignRequestWithSignerGroupId", async function testCreateSignRequestWithSignerGroupId(): Promise<any> {
    ...
    const fileToSign: FileFull = await uploadNewFile();
    const destinationFolder: FolderFull = await createNewFolder();
    const createdSignRequest: SignRequest = await client.signRequests.createSignRequest({ signers: [{ email: signer1Email, signerGroupId: "user" } satisfies SignRequestCreateSigner, { email: signer2Email, signerGroupId: "user" } satisfies SignRequestCreateSigner], parentFolder: new FolderMini({ id: destinationFolder.id }), sourceFiles: [new FileBase({ id: fileToSign.id })] } satisfies SignRequestCreateRequest);
