...
test("testGetTermsOfServiceUserStatuses", async function testGetTermsOfServiceUserStatuses(): Promise<any> {
    ...
    const user: UserFull = await client.users.createUser({ name: getUuid(), login: "".concat(getUuid(), "@boxdemo.com") as string, isPlatformAccessOnly: true } satisfies CreateUserRequestBody);
    const createdTosUserStatus: TermsOfServiceUserStatus = await client.termsOfServiceUserStatuses.createTermsOfServiceStatusForUser({ tos: new CreateTermsOfServiceStatusForUserRequestBodyTosField({ id: tos.id }), user: new CreateTermsOfServiceStatusForUserRequestBodyUserField({ id: user.id }), isAccepted: false } satisfies CreateTermsOfServiceStatusForUserRequestBody);
    if (!(createdTosUserStatus.isAccepted == false)) {
        throw new Error("Assertion failed");
    }
    if (!(toString(createdTosUserStatus.type) as string == "terms_of_service_user_status")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(createdTosUserStatus.tos!.type) as string == "terms_of_service")) {
        throw new Error("Assertion failed");
    }
    ...
    const updatedTosUserStatus: TermsOfServiceUserStatus = await client.termsOfServiceUserStatuses.updateTermsOfServiceStatusForUserById(createdTosUserStatus.id, { isAccepted: true } satisfies UpdateTermsOfServiceStatusForUserByIdRequestBody);
    if (!(updatedTosUserStatus.isAccepted == true)) {
        throw new Error("Assertion failed");
    }
    const listTosUserStatuses: TermsOfServiceUserStatuses = await client.termsOfServiceUserStatuses.getTermsOfServiceUserStatuses({ tosId: tos.id, userId: user.id } satisfies GetTermsOfServiceUserStatusesQueryParams);
