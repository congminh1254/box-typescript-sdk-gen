...
test("test_download_file", async function test_download_file(): Promise<any> {
    ...
    const downloadedFileContent: undefined | ByteStream = await client.downloads.downloadFile(uploadedFile.id);
...
test("test_get_download_url", async function test_get_download_url(): Promise<any> {
    ...
    const downloadUrl: string = await client.downloads.getDownloadFileUrl(uploadedFile.id);
...
test("test_change_download_url_with_interceptor", async function test_change_download_url_with_interceptor(): Promise<any> {
    ...
    function afterRequest(response: FetchResponse): FetchResponse {
        return { url: response.url, status: response.status, data: response.data, content: response.content, headers: { ...response.headers, ...{ ["location"]: "fake_location" } } } satisfies FetchResponse;
    }
    const clientWithInterceptor: BoxClient = client.withInterceptors([{ beforeRequest: emptyBeforeRequest, afterRequest: afterRequest } satisfies Interceptor]);
    await expect(async () => { await clientWithInterceptor.downloads.downloadFile(uploadedFile.id); }).rejects.toThrow();
    await client.files.deleteFileById(uploadedFile.id)
});
export {};
