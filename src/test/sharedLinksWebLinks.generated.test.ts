...
test("testSharedLinksWebLinks", async function testSharedLinksWebLinks(): Promise<any> {
    ...
    const webLink: WebLink = await client.webLinks.createWebLink({ url: "https://www.box.com", parent: { id: parent.id } satisfies CreateWebLinkRequestBodyParentField, name: getUuid(), description: "Weblink description" } satisfies CreateWebLinkRequestBody);
    ...
    const webLinkFromApi: WebLink = await client.sharedLinksWebLinks.getSharedLinkForWebLink(webLinkId, { fields: "shared_link" } satisfies GetSharedLinkForWebLinkQueryParams);
    ...
    const webLinkFromSharedLinkPassword: WebLink = await userClient.sharedLinksWebLinks.findWebLinkForSharedLink({} satisfies FindWebLinkForSharedLinkQueryParams, { boxapi: "".concat("shared_link=", webLinkFromApi.sharedLink!.url, "&shared_link_password=Secret123@") as string } satisfies FindWebLinkForSharedLinkHeadersInput);
    if (!(webLinkId == webLinkFromSharedLinkPassword.id)) {
        throw new Error("Assertion failed");
    }
    await expect(async () => { await userClient.sharedLinksWebLinks.findWebLinkForSharedLink({} satisfies FindWebLinkForSharedLinkQueryParams, { boxapi: "".concat("shared_link=", webLinkFromApi.sharedLink!.url, "&shared_link_password=incorrectPassword") as string } satisfies FindWebLinkForSharedLinkHeadersInput); }).rejects.toThrow();
    const updatedWebLink: WebLink = await client.sharedLinksWebLinks.updateSharedLinkOnWebLink(webLinkId, { sharedLink: { access: "collaborators" as UpdateSharedLinkOnWebLinkRequestBodySharedLinkAccessField } satisfies UpdateSharedLinkOnWebLinkRequestBodySharedLinkField } satisfies UpdateSharedLinkOnWebLinkRequestBody, { fields: "shared_link" } satisfies UpdateSharedLinkOnWebLinkQueryParams);
    if (!(toString(updatedWebLink.sharedLink!.access) as string == "collaborators")) {
        throw new Error("Assertion failed");
    }
    ...
    const webLinkFromApiAfterRemove: WebLink = await client.sharedLinksWebLinks.getSharedLinkForWebLink(webLinkId, { fields: "shared_link" } satisfies GetSharedLinkForWebLinkQueryParams);
