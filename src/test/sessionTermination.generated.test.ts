...
test("testSessionTerminationUser", async function testSessionTerminationUser(): Promise<any> {
    ...
    const result: SessionTerminationMessage = await client.sessionTermination.terminateUsersSessions({ userIds: [getEnvVar("USER_ID")], userLogins: [user.login!] } satisfies TerminateUsersSessionsRequestBody);
    if (!(result.message == "Request is successful, please check the admin events for the status of the job")) {
...
test("testSessionTerminationGroup", async function testSessionTerminationGroup(): Promise<any> {
    ...
    const group: GroupFull = await client.groups.createGroup({ name: groupName } satisfies CreateGroupRequestBody);
    const result: SessionTerminationMessage = await client.sessionTermination.terminateGroupsSessions({ groupIds: [group.id] } satisfies TerminateGroupsSessionsRequestBody);
    if (!(result.message == "Request is successful, please check the admin events for the status of the job")) {
        throw new Error("Assertion failed");
    }
