...
export async function getOrCreateStoragePolicyAssignment(clientInput: BoxClientInput, policyId: string, userId: string): Promise<StoragePolicyAssignment> {
    const client: BoxClient = new BoxClient({ auth: clientInput.auth, networkSession: clientInput.networkSession });
    const storagePolicyAssignments: StoragePolicyAssignments = await client.storagePolicyAssignments.getStoragePolicyAssignments({ resolvedForType: "user" as GetStoragePolicyAssignmentsQueryParamsResolvedForTypeField, resolvedForId: userId } satisfies GetStoragePolicyAssignmentsQueryParams);
    if (storagePolicyAssignments.entries!.length > 0) {
        if (toString(storagePolicyAssignments.entries![0].assignedTo!.type) as string == "user") {
            return storagePolicyAssignments.entries![0];
        }
    }
    const storagePolicyAssignment: StoragePolicyAssignment = await client.storagePolicyAssignments.createStoragePolicyAssignment({ storagePolicy: new CreateStoragePolicyAssignmentRequestBodyStoragePolicyField({ id: policyId }), assignedTo: { id: userId, type: "user" as CreateStoragePolicyAssignmentRequestBodyAssignedToTypeField } satisfies CreateStoragePolicyAssignmentRequestBodyAssignedToField } satisfies CreateStoragePolicyAssignmentRequestBody);
...
test("testGetStoragePolicyAssignments", async function testGetStoragePolicyAssignments(): Promise<any> {
    ...
    const newUser: UserFull = await client.users.createUser({ name: userName, isPlatformAccessOnly: true } satisfies CreateUserRequestBody);
    const storagePolicies: StoragePolicies = await client.storagePolicies.getStoragePolicies();
    const storagePolicy1: StoragePolicy = storagePolicies.entries![0];
    const storagePolicy2: StoragePolicy = storagePolicies.entries![1];
    const storagePolicyAssignment: StoragePolicyAssignment = await getOrCreateStoragePolicyAssignment(client, storagePolicy1.id, newUser.id);
    if (!(toString(storagePolicyAssignment.type) as string == "storage_policy_assignment")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(storagePolicyAssignment.assignedTo!.type) as string == "user")) {
        throw new Error("Assertion failed");
    }
    if (!(storagePolicyAssignment.assignedTo!.id == newUser.id)) {
        throw new Error("Assertion failed");
    }
    const getStoragePolicyAssignment: StoragePolicyAssignment = await client.storagePolicyAssignments.getStoragePolicyAssignmentById(storagePolicyAssignment.id);
    if (!(getStoragePolicyAssignment.id == storagePolicyAssignment.id)) {
        throw new Error("Assertion failed");
    }
    const updatedStoragePolicyAssignment: StoragePolicyAssignment = await client.storagePolicyAssignments.updateStoragePolicyAssignmentById(storagePolicyAssignment.id, { storagePolicy: new UpdateStoragePolicyAssignmentByIdRequestBodyStoragePolicyField({ id: storagePolicy2.id }) } satisfies UpdateStoragePolicyAssignmentByIdRequestBody);
