...
test("testCreateUpdateGetDeleteRetentionPolicy", async function testCreateUpdateGetDeleteRetentionPolicy(): Promise<any> {
    ...
    const retentionPolicyAssignment: RetentionPolicyAssignment = await client.retentionPolicyAssignments.createRetentionPolicyAssignment({ policyId: retentionPolicy.id, assignTo: { id: folder.id, type: "folder" as CreateRetentionPolicyAssignmentRequestBodyAssignToTypeField } satisfies CreateRetentionPolicyAssignmentRequestBodyAssignToField } satisfies CreateRetentionPolicyAssignmentRequestBody);
    ...
    if (!(toString(retentionPolicyAssignment.assignedTo!.type) as string == toString(folder.type) as string)) {
        throw new Error("Assertion failed");
    }
    const files: Files = await client.uploads.uploadFile({ attributes: { name: getUuid(), parent: { id: folder.id } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(10) } satisfies UploadFileRequestBody);
    const file: FileFull = files.entries![0];
    const newFiles: Files = await client.uploads.uploadFileVersion(file.id, { attributes: { name: file.name! } satisfies UploadFileVersionRequestBodyAttributesField, file: generateByteStream(20) } satisfies UploadFileVersionRequestBody);
    const newFile: FileFull = newFiles.entries![0];
    if (!(newFile.id == file.id)) {
        throw new Error("Assertion failed");
    }
    const fileVersionRetentions: FileVersionRetentions = await client.fileVersionRetentions.getFileVersionRetentions();
    const fileVersionRetentionsCount: number = fileVersionRetentions.entries!.length;
    if (!(fileVersionRetentionsCount >= 0)) {
        throw new Error("Assertion failed");
    }
    if (fileVersionRetentionsCount == 0) {
        await client.retentionPolicies.deleteRetentionPolicyById(retentionPolicy.id)
        await client.folders.deleteFolderById(folder.id, { queryParams: { recursive: true } satisfies DeleteFolderByIdQueryParams } satisfies DeleteFolderByIdOptionalsInput)
        return void 0;
    }
    const fileVersionRetention: FileVersionRetention = fileVersionRetentions.entries![0];
    const fileVersionRetentionById: FileVersionRetention = await client.fileVersionRetentions.getFileVersionRetentionById(fileVersionRetention.id!);
