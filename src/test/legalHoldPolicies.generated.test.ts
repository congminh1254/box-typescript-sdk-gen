...
test("testCreateNotOngoingLegalHoldPolicy", async function testCreateNotOngoingLegalHoldPolicy(): Promise<any> {
    ...
    const filterStartedAt: DateTime = dateTimeFromString("2021-01-01T00:00:00-08:00");
    const filterEndedAt: DateTime = dateTimeFromString("2022-01-01T00:00:00-08:00");
    const legalHoldPolicy: LegalHoldPolicy = await client.legalHoldPolicies.createLegalHoldPolicy({ policyName: legalHoldPolicyName, description: legalHoldDescription, isOngoing: false, filterStartedAt: filterStartedAt, filterEndedAt: filterEndedAt } satisfies CreateLegalHoldPolicyRequestBody);
    ...
    if (!(dateTimeToString(legalHoldPolicy.filterStartedAt!) == dateTimeToString(filterStartedAt))) {
        throw new Error("Assertion failed");
    }
    if (!(dateTimeToString(legalHoldPolicy.filterEndedAt!) == dateTimeToString(filterEndedAt))) {
        throw new Error("Assertion failed");
    }
...
test("testCreateUpdateGetDeleteLegalHoldPolicy", async function testCreateUpdateGetDeleteLegalHoldPolicy(): Promise<any> {
    ...
    const legalHoldPolicyById: LegalHoldPolicy = await client.legalHoldPolicies.getLegalHoldPolicyById(legalHoldPolicyId);
    if (!(legalHoldPolicyById.id == legalHoldPolicyId)) {
        throw new Error("Assertion failed");
    }
    const legalHoldPolicies: LegalHoldPolicies = await client.legalHoldPolicies.getLegalHoldPolicies();
    if (!(legalHoldPolicies.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const updatedLegalHoldPolicyName: string = getUuid();
    const updatedLegalHoldPolicy: LegalHoldPolicy = await client.legalHoldPolicies.updateLegalHoldPolicyById(legalHoldPolicyId, { requestBody: { policyName: updatedLegalHoldPolicyName } satisfies UpdateLegalHoldPolicyByIdRequestBody } satisfies UpdateLegalHoldPolicyByIdOptionalsInput);
