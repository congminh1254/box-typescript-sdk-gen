...
test("testAiStudioCRUD", async function testAiStudioCRUD(): Promise<any> {
    ...
    const createdAgent: AiSingleAgentResponseFull = await client.aiStudio.createAiAgent({ name: agentName, accessState: "enabled", ask: new AiStudioAgentAsk({ accessState: "enabled", description: "desc1" }) } satisfies CreateAiAgentInput);
    ...
    const retrievedAgent: AiSingleAgentResponseFull = await client.aiStudio.getAiAgentById(createdAgent.id, { queryParams: { fields: ["ask" as string] } satisfies GetAiAgentByIdQueryParams } satisfies GetAiAgentByIdOptionalsInput);
    ...
    const updatedAgent: AiSingleAgentResponseFull = await client.aiStudio.updateAiAgentById(createdAgent.id, { name: agentName, accessState: "enabled", ask: new AiStudioAgentAsk({ accessState: "disabled", description: "desc2" }) } satisfies CreateAiAgentInput);
    ...
    const agentsAfterDelete: AiMultipleAgentResponse = await client.aiStudio.getAiAgents();
