...
export class InterceptorThrowingError implements Interceptor {
     ...
     */
    afterRequest(response: FetchResponse): FetchResponse {
        return { url: response.url, status: 400, data: response.data, content: response.content, headers: response.headers } satisfies FetchResponse;
    }
}
export class InterceptorChangingResponse implements Interceptor {
    ...
    beforeRequest(optionsInput: FetchOptionsInput): FetchOptions {
        const options: FetchOptions = new FetchOptions({ url: optionsInput.url, method: optionsInput.method, params: optionsInput.params, headers: optionsInput.headers, data: optionsInput.data, fileStream: optionsInput.fileStream, multipartData: optionsInput.multipartData, contentType: optionsInput.contentType, responseFormat: optionsInput.responseFormat, auth: optionsInput.auth, networkSession: optionsInput.networkSession, cancellationToken: optionsInput.cancellationToken, followRedirects: optionsInput.followRedirects });
        return options;
    }
    /**
     * @param {FetchResponse} response
     ...
     */
    afterRequest(response: FetchResponse): FetchResponse {
        return { url: response.url, status: response.status, data: jsonToSerializedData("{\"id\": \"123\", \"type\": \"user\"}"), content: response.content, headers: response.headers } satisfies FetchResponse;
    }
}
test("testMakeRequestJsonCRUD", async function testMakeRequestJsonCRUD(): Promise<any> {
    const newFolderName: string = getUuid();
    const requestBodyPost: string = "".concat("{\"name\": \"", newFolderName, "\", \"parent\": { \"id\": \"0\"}}") as string;
    const createFolderResponse: FetchResponse = await client.makeRequest({ method: "post", url: "https://api.box.com/2.0/folders", data: jsonToSerializedData(requestBodyPost) } satisfies FetchOptionsInput);
    ...
    const requestBodyPut: string = "".concat("{\"name\": \"", updatedName, "\"}") as string;
    const updateFolderResponse: FetchResponse = await client.makeRequest({ method: "put", url: "".concat("https://api.box.com/2.0/folders/", getSdValueByKey(createdFolder, "id")) as string, data: jsonToSerializedData(requestBodyPut) } satisfies FetchOptionsInput);
    ...
    if (!(getSdValueByKey(updatedFolder, "id") == getSdValueByKey(createdFolder, "id"))) {
        throw new Error("Assertion failed");
    }
    const getFolderResponse: FetchResponse = await client.makeRequest({ url: "".concat("https://api.box.com/2.0/folders/", getSdValueByKey(createdFolder, "id")) as string, method: "GET" } satisfies FetchOptionsInput);
    ...
    if (!(getSdValueByKey(receivedFolder, "id") == getSdValueByKey(updatedFolder, "id"))) {
        throw new Error("Assertion failed");
    }
    const deleteFolderResponse: FetchResponse = await client.makeRequest({ url: "".concat("https://api.box.com/2.0/folders/", getSdValueByKey(receivedFolder, "id")) as string, method: "DELETE" } satisfies FetchOptionsInput);
...
test("testMakeRequestMultipart", async function testMakeRequestMultipart(): Promise<any> {
    const newFolderName: string = getUuid();
    const newFolder: FolderFull = await client.folders.createFolder({ name: newFolderName, parent: { id: "0" } satisfies CreateFolderRequestBodyParentField } satisfies CreateFolderRequestBody);
    ...
    const multipartAttributes: string = "".concat("{\"name\": \"", newFileName, "\", \"parent\": { \"id\":", newFolderId, "}}") as string;
    const uploadFileResponse: FetchResponse = await client.makeRequest({ method: "POST", url: "https://upload.box.com/api/2.0/files/content", contentType: "multipart/form-data", multipartData: [{ partName: "attributes", data: jsonToSerializedData(multipartAttributes) } satisfies MultipartItem, { partName: "file", fileStream: fileContentStream } satisfies MultipartItem] } satisfies FetchOptionsInput);
    if (!(uploadFileResponse.status == 201)) {
        throw new Error("Assertion failed");
    }
    await client.folders.deleteFolderById(newFolderId, { queryParams: { recursive: true } satisfies DeleteFolderByIdQueryParams } satisfies DeleteFolderByIdOptionalsInput)
});
test("testMakeRequestBinaryFormat", async function testMakeRequestBinaryFormat(): Promise<any> {
    ...
    const fileContentStream: ByteStream = generateByteStreamFromBuffer(fileBuffer);
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: newFileName, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: fileContentStream } satisfies UploadFileRequestBody);
    const uploadedFile: FileFull = uploadedFiles.entries![0];
    const downloadFileResponse: FetchResponse = await client.makeRequest({ method: "GET", url: "".concat("https://api.box.com/2.0/files/", uploadedFile.id, "/content") as string, responseFormat: "binary" as ResponseFormat } satisfies FetchOptionsInput);
...
test("testWithAsUserHeader", async function testWithAsUserHeader(): Promise<any> {
    ...
    const createdUser: UserFull = await client.users.createUser({ name: userName, isPlatformAccessOnly: true } satisfies CreateUserRequestBody);
...
test("testWithExtraHeaders", async function testWithExtraHeaders(): Promise<any> {
    const userName: string = getUuid();
    const createdUser: UserFull = await client.users.createUser({ name: userName, isPlatformAccessOnly: true } satisfies CreateUserRequestBody);
    const asUserClient: BoxClient = client.withExtraHeaders({ ["As-User"]: createdUser.id });
    const adminUser: UserFull = await client.users.getUserMe();
    if (!!(toString(adminUser.name) as string == userName)) {
        throw new Error("Assertion failed");
    }
...
test("testWithCustomBaseUrls", async function testWithCustomBaseUrls(): Promise<any> {
    const newBaseUrls: BaseUrls = new BaseUrls({ baseUrl: "https://box.com/", uploadUrl: "https://box.com/", oauth2Url: "https://box.com/" });
    const customBaseClient: BoxClient = client.withCustomBaseUrls(newBaseUrls);
    await expect(async () => { await customBaseClient.users.getUserMe(); }).rejects.toThrow();
});
test("testWithInterceptors", async function testWithInterceptors(): Promise<any> {
    ...
    const clientWithInterceptor: BoxClient = client.withInterceptors([new InterceptorAddingRoleToFields({})]);
    const newUser: UserFull = await clientWithInterceptor.users.getUserMe();
    if (!!(newUser.role == void 0)) {
        throw new Error("Assertion failed");
    }
    const clientWithTwoInterceptors: BoxClient = clientWithInterceptor.withInterceptors([new InterceptorChangingResponse({})]);
    const superNewUser: UserFull = await clientWithTwoInterceptors.users.getUserMe();
...
test("testWithFailingInterceptors", async function testWithFailingInterceptors(): Promise<any> {
    ...
    const clientWithInterceptor: BoxClient = client.withInterceptors([new InterceptorThrowingError({})]);
    await expect(async () => { await clientWithInterceptor.users.getUserMe(); }).rejects.toThrow();
});
export {};
