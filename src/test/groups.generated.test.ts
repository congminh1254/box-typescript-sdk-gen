...
test("test_create_get_delete_group", async function test_create_get_delete_group(): Promise<any> {
    ...
    const group: GroupFull = await client.groups.createGroup({ name: groupName, description: groupDescription } satisfies CreateGroupRequestBody);
    if (!(group.name == groupName)) {
        throw new Error("Assertion failed");
    }
    const groupById: GroupFull = await client.groups.getGroupById(group.id, { queryParams: { fields: ["id" as string, "name" as string, "description" as string, "group_type" as string] } satisfies GetGroupByIdQueryParams } satisfies GetGroupByIdOptionalsInput);
    ...
    const updatedGroup: GroupFull = await client.groups.updateGroupById(group.id, { requestBody: { name: updatedGroupName } satisfies UpdateGroupByIdRequestBody } satisfies UpdateGroupByIdOptionalsInput);
    if (!(updatedGroup.name == updatedGroupName)) {
        throw new Error("Assertion failed");
    }
    await client.groups.deleteGroupById(group.id)
    await expect(async () => { await client.groups.getGroupById(group.id); }).rejects.toThrow();
});
export {};
