...
export function getCcgAuth(): BoxCcgAuth {
    const ccgConfig: CcgConfig = new CcgConfig({ clientId: getEnvVar("CLIENT_ID"), clientSecret: getEnvVar("CLIENT_SECRET"), enterpriseId: getEnvVar("ENTERPRISE_ID") });
...
export function getDefaultClient(): BoxClient {
    const client: BoxClient = new BoxClient({ auth: isBrowser() ? getCcgAuth() : getJwtAuth() });
    return client;
}
export async function createNewFolder(): Promise<FolderFull> {
    const client: BoxClient = getDefaultClient();
    const newFolderName: string = getUuid();
    return await client.folders.createFolder({ name: newFolderName, parent: { id: "0" } satisfies CreateFolderRequestBodyParentField } satisfies CreateFolderRequestBody);
}
...
export async function getOrCreateTermsOfServices(): Promise<TermsOfService> {
    ...
    return await client.termsOfServices.createTermsOfService({ status: "disabled" as CreateTermsOfServiceRequestBodyStatusField, tosType: "managed" as CreateTermsOfServiceRequestBodyTosTypeField, text: "Test TOS" } satisfies CreateTermsOfServiceRequestBody);
}
export async function getOrCreateClassification(classificationTemplateInput: ClassificationTemplateInput): Promise<ClassificationTemplateFieldsOptionsField> {
    const classificationTemplate: ClassificationTemplate = new ClassificationTemplate({ id: classificationTemplateInput.id, type: classificationTemplateInput.type, scope: classificationTemplateInput.scope, templateKey: classificationTemplateInput.templateKey, displayName: classificationTemplateInput.displayName, hidden: classificationTemplateInput.hidden, copyInstanceOnItemCopy: classificationTemplateInput.copyInstanceOnItemCopy, fields: classificationTemplateInput.fields });
    const client: BoxClient = getDefaultClient();
    const classifications: readonly ClassificationTemplateFieldsOptionsField[] = classificationTemplate.fields[0].options;
    const currentNumberOfClassifications: number = classifications.length;
    if (currentNumberOfClassifications == 0) {
        const classificationTemplateWithNewClassification: ClassificationTemplate = await client.classifications.addClassification([new AddClassificationRequestBody({ data: { key: getUuid(), staticConfig: { classification: { colorId: 3, classificationDefinition: "Some description" } satisfies AddClassificationRequestBodyDataStaticConfigClassificationField } satisfies AddClassificationRequestBodyDataStaticConfigField } satisfies AddClassificationRequestBodyDataField })]);
...
export async function getOrCreateClassificationTemplate(): Promise<ClassificationTemplate> {
    ...
    try {
        ...
        return await client.classifications.createClassificationTemplate({ fields: [new CreateClassificationTemplateRequestBodyFieldsField({ options: [] })] } satisfies CreateClassificationTemplateRequestBodyInput);
    }
    finally {
    }
}
export async function getOrCreateShieldInformationBarrier(clientInput: BoxClientInput, enterpriseId: string): Promise<ShieldInformationBarrier> {
    const client: BoxClient = new BoxClient({ auth: clientInput.auth, networkSession: clientInput.networkSession });
    ...
    if (numberOfBarriers == 0) {
        return await client.shieldInformationBarriers.createShieldInformationBarrier({ enterprise: { id: enterpriseId } satisfies EnterpriseBase } satisfies CreateShieldInformationBarrierRequestBody);
    }
