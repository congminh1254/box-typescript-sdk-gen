...
export async function uploadFile(fileName: string, fileStream: ByteStream): Promise<FileFull> {
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: fileName, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: fileStream } satisfies UploadFileRequestBody);
...
test("testGetFileThumbnailUrl", async function testGetFileThumbnailUrl(): Promise<any> {
    ...
    const thumbnailFile: FileFull = await uploadFile(thumbnailFileName, thumbnailContentStream);
    const downloadUrl: string = await client.files.getFileThumbnailUrl(thumbnailFile.id, "png" as GetFileThumbnailUrlExtension);
...
test("testGetFileThumbnail", async function testGetFileThumbnail(): Promise<any> {
    ...
    const thumbnailContentStream: ByteStream = generateByteStream(1024 * 1024);
    const thumbnailFile: FileFull = await uploadFile(thumbnailFileName, thumbnailContentStream);
    const thumbnail: undefined | ByteStream = await client.files.getFileThumbnailById(thumbnailFile.id, "png" as GetFileThumbnailByIdExtension);
    if (!!(bufferEquals(await readByteStream(thumbnail!), await readByteStream(thumbnailContentStream)) == true)) {
        throw new Error("Assertion failed");
    }
    await client.files.deleteFileById(thumbnailFile.id)
});
test("testGetFileFullExtraFields", async function testGetFileFullExtraFields(): Promise<any> {
    ...
    const file: FileFull = await client.files.getFileById(uploadedFile.id, { queryParams: { fields: ["is_externally_owned" as string, "has_collaborations" as string] } satisfies GetFileByIdQueryParams } satisfies GetFileByIdOptionalsInput);
...
test("testCreateGetAndDeleteFile", async function testCreateGetAndDeleteFile(): Promise<any> {
    ...
    const uploadedFile: FileFull = await uploadFile(newFileName, updatedContentStream);
    const file: FileFull = await client.files.getFileById(uploadedFile.id);
    await expect(async () => { await client.files.getFileById(uploadedFile.id, { queryParams: { fields: ["name" as string] } satisfies GetFileByIdQueryParams, headers: new GetFileByIdHeaders({ extraHeaders: { ["if-none-match"]: file.etag! } }) } satisfies GetFileByIdOptionalsInput); }).rejects.toThrow();
    if (!(file.name == newFileName)) {
        throw new Error("Assertion failed");
    }
    await client.files.deleteFileById(uploadedFile.id)
    const trashedFile: TrashFile = await client.trashedFiles.getTrashedFileById(uploadedFile.id);
...
test("testUpdateFile", async function testUpdateFile(): Promise<any> {
    ...
    const updatedFile: FileFull = await client.files.updateFileById(fileToUpdate.id, { requestBody: { name: updatedName, description: "Updated description" } satisfies UpdateFileByIdRequestBody } satisfies UpdateFileByIdOptionalsInput);
...
test("testFileLock", async function testFileLock(): Promise<any> {
    ...
    const fileWithLock: FileFull = await client.files.updateFileById(file.id, { requestBody: { lock: { access: "lock" as UpdateFileByIdRequestBodyLockAccessField } satisfies UpdateFileByIdRequestBodyLockField } satisfies UpdateFileByIdRequestBody, queryParams: { fields: ["lock" as string] } satisfies UpdateFileByIdQueryParams } satisfies UpdateFileByIdOptionalsInput);
    if (!!(fileWithLock.lock == void 0)) {
        throw new Error("Assertion failed");
    }
    const fileWithoutLock: FileFull = await client.files.updateFileById(file.id, { requestBody: { lock: createNull() } satisfies UpdateFileByIdRequestBody, queryParams: { fields: ["lock" as string] } satisfies UpdateFileByIdQueryParams } satisfies UpdateFileByIdOptionalsInput);
...
test("testCopyFile", async function testCopyFile(): Promise<any> {
    ...
    const copiedFile: FileFull = await client.files.copyFile(fileOrigin.id, { parent: { id: "0" } satisfies CopyFileRequestBodyParentField, name: copiedFileName } satisfies CopyFileRequestBody);
