...
test("testTransferUserContent", async function testTransferUserContent(): Promise<any> {
    ...
    const newUser: UserFull = await client.users.createUser({ name: newUserName, isPlatformAccessOnly: true } satisfies CreateUserRequestBody);
    const currentUser: UserFull = await client.users.getUserMe();
    const transferedFolder: FolderFull = await client.transfer.transferOwnedFolder(newUser.id, { ownedBy: { id: currentUser.id } satisfies TransferOwnedFolderRequestBodyOwnedByField } satisfies TransferOwnedFolderRequestBody, { queryParams: { notify: false } satisfies TransferOwnedFolderQueryParams } satisfies TransferOwnedFolderOptionalsInput);
    if (!(transferedFolder.ownedBy!.id == currentUser.id)) {
        throw new Error("Assertion failed");
    }
    await client.folders.deleteFolderById(transferedFolder.id, { queryParams: { recursive: true } satisfies DeleteFolderByIdQueryParams } satisfies DeleteFolderByIdOptionalsInput)
    await client.users.deleteUserById(newUser.id, { queryParams: { notify: false, force: true } satisfies DeleteUserByIdQueryParams } satisfies DeleteUserByIdOptionalsInput)
});
export {};
