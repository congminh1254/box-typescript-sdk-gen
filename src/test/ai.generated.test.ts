...
test("testAskAISingleItem", async function testAskAISingleItem(): Promise<any> {
    ...
    const response: AiResponseFull = await client.ai.createAiAsk({ mode: "single_item_qa" as AiAskModeField, prompt: "which direction sun rises", items: [new AiItemBase({ id: fileToAsk.id, type: "file" as AiItemBaseTypeField, content: "Sun rises in the East" })] } satisfies AiAsk);
...
test("testAskAIMultipleItems", async function testAskAIMultipleItems(): Promise<any> {
    ...
    const response: AiResponseFull = await client.ai.createAiAsk({ mode: "multiple_item_qa" as AiAskModeField, prompt: "Which direction sun rises?", items: [new AiItemBase({ id: fileToAsk1.id, type: "file" as AiItemBaseTypeField, content: "Earth goes around the sun" }), new AiItemBase({ id: fileToAsk2.id, type: "file" as AiItemBaseTypeField, content: "Sun rises in the East in the morning" })] } satisfies AiAsk);
    if (!(response.answer.includes("East") as boolean)) {
        throw new Error("Assertion failed");
    }
    if (!(response.completionReason == "done")) {
...
test("testAIExtractStructuredWithMetadataTemplate", async function testAIExtractStructuredWithMetadataTemplate(): Promise<any> {
    ...
    const response: AiExtractResponse = await client.ai.createAiExtractStructured({ metadataTemplate: { templateKey: templateKey, scope: "enterprise" } satisfies AiExtractStructuredMetadataTemplateField, items: [new AiItemBase({ id: file.id })] } satisfies AiExtractStructured);
    if (!(toString(getValueFromObjectRawData(response, "firstName")) as string == "John")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "lastName")) as string == "Doe")) {
    ...
    if (!(toString(getValueFromObjectRawData(response, "dateOfBirth")) as string == "1990-07-04T00:00:00Z")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "age")) as string == "34")) {
        throw new Error("Assertion failed");
