...
test("testAskAISingleItem", async function testAskAISingleItem(): Promise<any> {
    ...
    const response: undefined | AiResponseFull = await client.ai.createAiAsk({ mode: "single_item_qa" as AiAskModeField, prompt: "which direction sun rises", items: [{ id: fileToAsk.id, type: "file" as AiItemAskTypeField, content: "Sun rises in the East" } satisfies AiItemAsk] } satisfies AiAsk);
...
test("testAskAIMultipleItems", async function testAskAIMultipleItems(): Promise<any> {
    ...
    const response: undefined | AiResponseFull = await client.ai.createAiAsk({ mode: "multiple_item_qa" as AiAskModeField, prompt: "Which direction sun rises?", items: [{ id: fileToAsk1.id, type: "file" as AiItemAskTypeField, content: "Earth goes around the sun" } satisfies AiItemAsk, { id: fileToAsk2.id, type: "file" as AiItemAskTypeField, content: "Sun rises in the East in the morning" } satisfies AiItemAsk] } satisfies AiAsk);
    if (!(response!.answer.includes("East") as boolean)) {
        throw new Error("Assertion failed");
    }
    if (!(response!.completionReason == "done")) {
...
test("testGettingAIAskAgentConfig", async function testGettingAIAskAgentConfig(): Promise<any> {
    const aiAskConfig: AiAgentAskOrAiAgentExtractOrAiAgentExtractStructuredOrAiAgentTextGen = await client.ai.getAiAgentDefaultConfig({ mode: "ask" as GetAiAgentDefaultConfigQueryParamsModeField, language: "en-US" } satisfies GetAiAgentDefaultConfigQueryParams);
...
test("testGettingAITextGenAgentConfig", async function testGettingAITextGenAgentConfig(): Promise<any> {
    const aiTextGenConfig: AiAgentAskOrAiAgentExtractOrAiAgentExtractStructuredOrAiAgentTextGen = await client.ai.getAiAgentDefaultConfig({ mode: "text_gen" as GetAiAgentDefaultConfigQueryParamsModeField, language: "en-US" } satisfies GetAiAgentDefaultConfigQueryParams);
...
test("testAIExtract", async function testAIExtract(): Promise<any> {
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: "".concat(getUuid(), ".txt") as string, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: stringToByteStream("My name is John Doe. I live in San Francisco. I was born in 1990. I work at Box.") } satisfies UploadFileRequestBody);
    const file: FileFull = uploadedFiles.entries![0];
    await delayInSeconds(5)
    const response: AiResponse = await client.ai.createAiExtract({ prompt: "firstName, lastName, location, yearOfBirth, company", items: [new AiItemBase({ id: file.id })] } satisfies AiExtract);
    const expectedResponse: string = "{\"firstName\": \"John\", \"lastName\": \"Doe\", \"location\": \"San Francisco\", \"yearOfBirth\": \"1990\", \"company\": \"Box\"}";
...
test("testAIExtractStructuredWithFields", async function testAIExtractStructuredWithFields(): Promise<any> {
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: "".concat(getUuid(), ".txt") as string, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: stringToByteStream("My name is John Doe. I was born in 4th July 1990. I am 34 years old. My hobby is guitar.") } satisfies UploadFileRequestBody);
    const file: FileFull = uploadedFiles.entries![0];
    await delayInSeconds(5)
    const response: AiExtractStructuredResponse = await client.ai.createAiExtractStructured({ fields: [{ key: "firstName", displayName: "First name", description: "Person first name", prompt: "What is the your first name?", type: "string" } satisfies AiExtractStructuredFieldsField, { key: "lastName", displayName: "Last name", description: "Person last name", prompt: "What is the your last name?", type: "string" } satisfies AiExtractStructuredFieldsField, { key: "dateOfBirth", displayName: "Birth date", description: "Person date of birth", prompt: "What is the date of your birth?", type: "date" } satisfies AiExtractStructuredFieldsField, { key: "age", displayName: "Age", description: "Person age", prompt: "How old are you?", type: "float" } satisfies AiExtractStructuredFieldsField, { key: "hobby", displayName: "Hobby", description: "Person hobby", prompt: "What is your hobby?", type: "multiSelect", options: [{ key: "guitar" } satisfies AiExtractStructuredFieldsOptionsField, { key: "books" } satisfies AiExtractStructuredFieldsOptionsField] } satisfies AiExtractStructuredFieldsField], items: [new AiItemBase({ id: file.id })] } satisfies AiExtractStructured);
    if (!(toString(getValueFromObjectRawData(response, "answer.hobby")) as string == ["guitar"].map(toString).join(",") as string)) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "answer.firstName")) as string == "John")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "answer.lastName")) as string == "Doe")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "answer.dateOfBirth")) as string == "1990-07-04")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "answer.age")) as string == "34")) {
        throw new Error("Assertion failed");
    }
    if (!(response.completionReason == "done")) {
        throw new Error("Assertion failed");
...
test("testAIExtractStructuredWithMetadataTemplate", async function testAIExtractStructuredWithMetadataTemplate(): Promise<any> {
    ...
    const template: MetadataTemplate = await client.metadataTemplates.createMetadataTemplate({ scope: "enterprise", displayName: templateKey, templateKey: templateKey, fields: [{ key: "firstName", displayName: "First name", description: "Person first name", type: "string" as CreateMetadataTemplateRequestBodyFieldsTypeField } satisfies CreateMetadataTemplateRequestBodyFieldsField, { key: "lastName", displayName: "Last name", description: "Person last name", type: "string" as CreateMetadataTemplateRequestBodyFieldsTypeField } satisfies CreateMetadataTemplateRequestBodyFieldsField, { key: "dateOfBirth", displayName: "Birth date", description: "Person date of birth", type: "date" as CreateMetadataTemplateRequestBodyFieldsTypeField } satisfies CreateMetadataTemplateRequestBodyFieldsField, { key: "age", displayName: "Age", description: "Person age", type: "float" as CreateMetadataTemplateRequestBodyFieldsTypeField } satisfies CreateMetadataTemplateRequestBodyFieldsField, { key: "hobby", displayName: "Hobby", description: "Person hobby", type: "multiSelect" as CreateMetadataTemplateRequestBodyFieldsTypeField, options: [{ key: "guitar" } satisfies CreateMetadataTemplateRequestBodyFieldsOptionsField, { key: "books" } satisfies CreateMetadataTemplateRequestBodyFieldsOptionsField] } satisfies CreateMetadataTemplateRequestBodyFieldsField] } satisfies CreateMetadataTemplateRequestBody);
    const response: AiExtractStructuredResponse = await client.ai.createAiExtractStructured({ metadataTemplate: { templateKey: templateKey, scope: "enterprise" } satisfies AiExtractStructuredMetadataTemplateField, items: [new AiItemBase({ id: file.id })] } satisfies AiExtractStructured);
    if (!(toString(getValueFromObjectRawData(response, "answer.firstName")) as string == "John")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "answer.lastName")) as string == "Doe")) {
    ...
    if (!(toString(getValueFromObjectRawData(response, "answer.dateOfBirth")) as string == "1990-07-04T00:00:00Z")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(getValueFromObjectRawData(response, "answer.age")) as string == "34")) {
        throw new Error("Assertion failed");
