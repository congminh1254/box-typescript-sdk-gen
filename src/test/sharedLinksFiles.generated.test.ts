...
test("testSharedLinksFiles", async function testSharedLinksFiles(): Promise<any> {
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: getUuid(), parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(10) } satisfies UploadFileRequestBody);
    ...
    const fileFromApi: FileFull = await client.sharedLinksFiles.getSharedLinkForFile(fileId, { fields: "shared_link" } satisfies GetSharedLinkForFileQueryParams);
    ...
    const fileFromSharedLinkPassword: FileFull = await userClient.sharedLinksFiles.findFileForSharedLink({} satisfies FindFileForSharedLinkQueryParams, { boxapi: "".concat("shared_link=", fileFromApi.sharedLink!.url, "&shared_link_password=Secret123@") as string } satisfies FindFileForSharedLinkHeadersInput);
    if (!(fileId == fileFromSharedLinkPassword.id)) {
        throw new Error("Assertion failed");
    }
    await expect(async () => { await userClient.sharedLinksFiles.findFileForSharedLink({} satisfies FindFileForSharedLinkQueryParams, { boxapi: "".concat("shared_link=", fileFromApi.sharedLink!.url, "&shared_link_password=incorrectPassword") as string } satisfies FindFileForSharedLinkHeadersInput); }).rejects.toThrow();
    const updatedFile: FileFull = await client.sharedLinksFiles.updateSharedLinkOnFile(fileId, { sharedLink: { access: "collaborators" as UpdateSharedLinkOnFileRequestBodySharedLinkAccessField } satisfies UpdateSharedLinkOnFileRequestBodySharedLinkField } satisfies UpdateSharedLinkOnFileRequestBody, { fields: "shared_link" } satisfies UpdateSharedLinkOnFileQueryParams);
    if (!(toString(updatedFile.sharedLink!.access) as string == "collaborators")) {
        throw new Error("Assertion failed");
    }
    ...
    const fileFromApiAfterRemove: FileFull = await client.sharedLinksFiles.getSharedLinkForFile(fileId, { fields: "shared_link" } satisfies GetSharedLinkForFileQueryParams);
