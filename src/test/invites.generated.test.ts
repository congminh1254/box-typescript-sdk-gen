...
test("testInvites", async function testInvites(): Promise<any> {
    ...
    const currentUser: UserFull = await client.users.getUserMe({ fields: ["enterprise" as string] } satisfies GetUserMeQueryParams);
    const email: string = getEnvVar("BOX_EXTERNAL_USER_EMAIL");
    const invitation: Invite = await client.invites.createInvite({ enterprise: { id: currentUser.enterprise!.id! } satisfies CreateInviteRequestBodyEnterpriseField, actionableBy: { login: email } satisfies CreateInviteRequestBodyActionableByField } satisfies CreateInviteRequestBody);
    ...
    const getInvitation: Invite = await client.invites.getInviteById(invitation.id);
