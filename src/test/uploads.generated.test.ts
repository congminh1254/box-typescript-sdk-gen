...
test("testUploadFileAndFileVersion", async function testUploadFileAndFileVersion(): Promise<any> {
    ...
    const uploadedFilesVersion: Files = await client.uploads.uploadFileVersion(uploadedFile.id, { attributes: { name: newFileVersionName } satisfies UploadFileVersionRequestBodyAttributesField, file: newFileContentStream } satisfies UploadFileVersionRequestBody);
...
test("testRequestCancellation", async function testRequestCancellation(): Promise<any> {
    ...
    await expect(async () => { await client.uploads.uploadFile({ attributes: { name: fileName, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: fileByteStream } satisfies UploadFileRequestBody, { queryParams: {} satisfies UploadFileQueryParams, headers: new UploadFileHeaders({}), cancellationToken: cancellationToken } satisfies UploadFileOptionalsInput); }).rejects.toThrow();
});
test("testUploadFileWithPreflightCheck", async function testUploadFileWithPreflightCheck(): Promise<any> {
    const newFileName: string = getUuid();
    const fileContentStream: ByteStream = generateByteStream(1024 * 1024);
    await expect(async () => { await client.uploads.uploadWithPreflightCheck({ attributes: { name: newFileName, size: -1, parent: { id: "0" } satisfies UploadWithPreflightCheckRequestBodyAttributesParentField } satisfies UploadWithPreflightCheckRequestBodyAttributesField, file: fileContentStream } satisfies UploadWithPreflightCheckRequestBody); }).rejects.toThrow();
    const uploadFilesWithPreflight: Files = await client.uploads.uploadWithPreflightCheck({ attributes: { name: newFileName, size: 1024 * 1024, parent: { id: "0" } satisfies UploadWithPreflightCheckRequestBodyAttributesParentField } satisfies UploadWithPreflightCheckRequestBodyAttributesField, file: fileContentStream } satisfies UploadWithPreflightCheckRequestBody);
    ...
    await expect(async () => { await client.uploads.uploadWithPreflightCheck({ attributes: { name: newFileName, size: 1024 * 1024, parent: { id: "0" } satisfies UploadWithPreflightCheckRequestBodyAttributesParentField } satisfies UploadWithPreflightCheckRequestBodyAttributesField, file: fileContentStream } satisfies UploadWithPreflightCheckRequestBody); }).rejects.toThrow();
    await client.files.deleteFileById(file.id)
});
test("testPreflightCheck", async function testPreflightCheck(): Promise<any> {
    const newFileName: string = getUuid();
    const preflightCheckResult: UploadUrl = await client.uploads.preflightFileUploadCheck({ name: newFileName, size: 1024 * 1024, parent: { id: "0" } satisfies PreflightFileUploadCheckRequestBodyParentField } satisfies PreflightFileUploadCheckRequestBody);
