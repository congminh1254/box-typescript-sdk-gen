...
test("testGlobalFileMetadata", async function testGlobalFileMetadata(): Promise<any> {
    ...
    const fileMetadata: Metadatas = await client.fileMetadata.getFileMetadata(file.id);
    if (!(fileMetadata.entries!.length == 0)) {
        throw new Error("Assertion failed");
    }
    const createdMetadata: MetadataFull = await client.fileMetadata.createFileMetadataById(file.id, "global" as CreateFileMetadataByIdScope, "properties", { ["abc"]: "xyz" });
    ...
    const receivedMetadata: MetadataFull = await client.fileMetadata.getFileMetadataById(file.id, "global" as GetFileMetadataByIdScope, "properties");
    if (!(toString(receivedMetadata.extraData!.abc) as string == "xyz")) {
        throw new Error("Assertion failed");
    }
    const newValue: string = "bar";
    await client.fileMetadata.updateFileMetadataById(file.id, "global" as UpdateFileMetadataByIdScope, "properties", [{ op: "replace" as UpdateFileMetadataByIdRequestBodyOpField, path: "/abc", value: newValue } satisfies UpdateFileMetadataByIdRequestBody])
    const receivedUpdatedMetadata: MetadataFull = await client.fileMetadata.getFileMetadataById(file.id, "global" as GetFileMetadataByIdScope, "properties");
    if (!(toString(receivedUpdatedMetadata.extraData!.abc) as string == newValue)) {
        throw new Error("Assertion failed");
    }
    await client.fileMetadata.deleteFileMetadataById(file.id, "global" as DeleteFileMetadataByIdScope, "properties")
    await expect(async () => { await client.fileMetadata.getFileMetadataById(file.id, "global" as GetFileMetadataByIdScope, "properties"); }).rejects.toThrow();
...
test("testEnterpriseFileMetadata", async function testEnterpriseFileMetadata(): Promise<any> {
    ...
    const createdMetadata: MetadataFull = await client.fileMetadata.createFileMetadataById(file.id, "enterprise" as CreateFileMetadataByIdScope, templateKey, { ["name"]: "John", ["age"]: 23, ["birthDate"]: "2001-01-03T02:20:50.520Z", ["countryCode"]: "US", ["sports"]: ["basketball", "tennis"] });
