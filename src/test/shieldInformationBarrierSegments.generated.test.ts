...
test("testShieldInformationBarrierSegments", async function testShieldInformationBarrierSegments(): Promise<any> {
    ...
    const segments: ShieldInformationBarrierSegments = await client.shieldInformationBarrierSegments.getShieldInformationBarrierSegments({ shieldInformationBarrierId: barrierId } satisfies GetShieldInformationBarrierSegmentsQueryParams);
    if (!(segments.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const segmentId: string = segment.id!;
    const segmentFromApi: ShieldInformationBarrierSegment = await client.shieldInformationBarrierSegments.getShieldInformationBarrierSegmentById(segmentId);
    if (!(toString(segmentFromApi.type!) as string == "shield_information_barrier_segment")) {
        throw new Error("Assertion failed");
    }
    ...
    const updatedSegmentDescription: string = "updated barrier segment description";
    const updatedSegment: ShieldInformationBarrierSegment = await client.shieldInformationBarrierSegments.updateShieldInformationBarrierSegmentById(segmentId, { requestBody: { description: updatedSegmentDescription } satisfies UpdateShieldInformationBarrierSegmentByIdRequestBody } satisfies UpdateShieldInformationBarrierSegmentByIdOptionalsInput);
    ...
    await expect(async () => { await client.shieldInformationBarrierSegments.getShieldInformationBarrierSegmentById(segmentId); }).rejects.toThrow();
});
export {};
