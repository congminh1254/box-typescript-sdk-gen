...
test("test_create_update_get_delete_user", async function test_create_update_get_delete_user(): Promise<any> {
    ...
    const updatedUser: UserFull = await client.users.updateUserById(user.id, { requestBody: { name: updatedUserName } satisfies UpdateUserByIdRequestBody } satisfies UpdateUserByIdOptionalsInput);
...
test("test_user_notification_email", async function test_user_notification_email(): Promise<any> {
    ...
    const userLogin: string = "".concat(getUuid(), "@gmail.com") as string;
    const user: UserFull = await client.users.createUser({ name: userName, login: userLogin, isPlatformAccessOnly: true } satisfies CreateUserRequestBody);
    const updatedWithNotificationEmail: UserFull = await client.users.updateUserById(user.id, { requestBody: { notificationEmail: { email: userLogin } satisfies UpdateUserByIdRequestBodyNotificationEmailField } satisfies UpdateUserByIdRequestBody } satisfies UpdateUserByIdOptionalsInput);
    if (!!(updatedWithNotificationEmail.notificationEmail == void 0)) {
        throw new Error("Assertion failed");
    }
    const updatedWithoutNotificationEmail: UserFull = await client.users.updateUserById(user.id, { requestBody: { notificationEmail: createNull() } satisfies UpdateUserByIdRequestBody } satisfies UpdateUserByIdOptionalsInput);
