...
test("testGlobalFolderMetadata", async function testGlobalFolderMetadata(): Promise<any> {
    ...
    const folderMetadata: Metadatas = await client.folderMetadata.getFolderMetadata(folder.id);
    if (!(folderMetadata.entries!.length == 0)) {
        throw new Error("Assertion failed");
    }
    const createdMetadata: MetadataFull = await client.folderMetadata.createFolderMetadataById(folder.id, "global" as CreateFolderMetadataByIdScope, "properties", { ["abc"]: "xyz" });
    ...
    const receivedMetadata: MetadataFull = await client.folderMetadata.getFolderMetadataById(folder.id, "global" as GetFolderMetadataByIdScope, "properties");
    if (!(toString(receivedMetadata.extraData!.abc) as string == "xyz")) {
        throw new Error("Assertion failed");
    }
    const newValue: string = "bar";
    await client.folderMetadata.updateFolderMetadataById(folder.id, "global" as UpdateFolderMetadataByIdScope, "properties", [{ op: "replace" as UpdateFolderMetadataByIdRequestBodyOpField, path: "/abc", value: newValue } satisfies UpdateFolderMetadataByIdRequestBody])
    const receivedUpdatedMetadata: MetadataFull = await client.folderMetadata.getFolderMetadataById(folder.id, "global" as GetFolderMetadataByIdScope, "properties");
    if (!(toString(receivedUpdatedMetadata.extraData!.abc) as string == newValue)) {
        throw new Error("Assertion failed");
    }
    await client.folderMetadata.deleteFolderMetadataById(folder.id, "global" as DeleteFolderMetadataByIdScope, "properties")
    await expect(async () => { await client.folderMetadata.getFolderMetadataById(folder.id, "global" as GetFolderMetadataByIdScope, "properties"); }).rejects.toThrow();
...
test("testEnterpriseFolderMetadata", async function testEnterpriseFolderMetadata(): Promise<any> {
    ...
    const createdMetadata: MetadataFull = await client.folderMetadata.createFolderMetadataById(folder.id, "enterprise" as CreateFolderMetadataByIdScope, templateKey, { ["name"]: "John", ["age"]: 23, ["birthDate"]: "2001-01-03T02:20:50.520Z", ["countryCode"]: "US", ["sports"]: ["basketball", "tennis"] });
    ...
    if (!(toString(createdMetadata.extraData!.birthDate) as string == "2001-01-03T02:20:50.520Z")) {
        throw new Error("Assertion failed");
    }
    ...
    await client.metadataTemplates.deleteMetadataTemplate("enterprise" as DeleteMetadataTemplateScope, templateKey)
    await client.folders.deleteFolderById(folder.id)
});
export {};
