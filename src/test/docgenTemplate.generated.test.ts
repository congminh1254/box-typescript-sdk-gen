...
test("testDocgenTemplateCRUD", async function testDocgenTemplateCRUD(): Promise<any> {
    ...
    const createdDocgenTemplate: DocGenTemplateBaseV2025R0 = await client.docgenTemplate.createDocgenTemplateV2025R0({ file: new FileReferenceV2025R0({ id: file.id }) } satisfies DocGenTemplateCreateRequestV2025R0);
    const docgenTemplates: DocGenTemplatesV2025R0 = await client.docgenTemplate.getDocgenTemplatesV2025R0();
    if (!(docgenTemplates.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const fetchedDocgenTemplate: DocGenTemplateV2025R0 = await client.docgenTemplate.getDocgenTemplateByIdV2025R0(createdDocgenTemplate.file!.id);
    if (!(fetchedDocgenTemplate.file!.id == createdDocgenTemplate.file!.id)) {
        throw new Error("Assertion failed");
    }
    const docgenTemplateTags: DocGenTagsV2025R0 = await client.docgenTemplate.getDocgenTemplateTagsV2025R0(fetchedDocgenTemplate.file!.id);
    const docgenTemplateJobs: DocGenJobsV2025R0 = await client.docgenTemplate.getDocgenTemplateJobByIdV2025R0(fetchedDocgenTemplate.file!.id);
