...
test("test_skills_cards_CRUD", async function test_skills_cards_CRUD(): Promise<any> {
    ...
    const skillCardsMetadata: SkillCardsMetadata = await client.skills.createBoxSkillCardsOnFile(file.id, { cards: [{ type: "skill_card", skillCardType: "keyword", skillCardTitle: { code: "license-plates", message: titleMessage }, skill: { id: skillId, type: "service" }, invocation: { id: invocationId, type: "skill_invocation" }, entries: [{ text: "DN86 BOX" }] } satisfies KeywordSkillCardOrStatusSkillCardOrTimelineSkillCardOrTranscriptSkillCard] } satisfies CreateBoxSkillCardsOnFileRequestBody);
    ...
    const updatedSkillCardsMetadata: SkillCardsMetadata = await client.skills.updateBoxSkillCardsOnFile(file.id, [{ op: "replace" as UpdateBoxSkillCardsOnFileRequestBodyOpField, path: "/cards/0", value: new KeywordSkillCard({ type: "skill_card" as KeywordSkillCardTypeField, skillCardType: "keyword" as KeywordSkillCardSkillCardTypeField, skillCardTitle: { code: "license-plates", message: updatedTitleMessage } satisfies KeywordSkillCardSkillCardTitleField, skill: new KeywordSkillCardSkillField({ id: skillId, type: "service" as KeywordSkillCardSkillTypeField }), invocation: new KeywordSkillCardInvocationField({ id: invocationId, type: "skill_invocation" as KeywordSkillCardInvocationTypeField }), entries: [{ text: "DN86 BOX" } satisfies KeywordSkillCardEntriesField] }) } satisfies UpdateBoxSkillCardsOnFileRequestBody]);
    if (!(updatedSkillCardsMetadata.cards![0].skill.id == skillId)) {
        throw new Error("Assertion failed");
    }
    if (!(updatedSkillCardsMetadata.cards![0].skillCardTitle!.message! == updatedTitleMessage)) {
        throw new Error("Assertion failed");
    }
    const receivedSkillCardsMetadata: SkillCardsMetadata = await client.skills.getBoxSkillCardsOnFile(file.id);
    if (!(updatedSkillCardsMetadata.cards![0].skill.id == skillId)) {
        throw new Error("Assertion failed");
    }
