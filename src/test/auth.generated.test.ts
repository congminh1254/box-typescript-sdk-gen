...
export async function getAccessToken(): Promise<AccessToken> {
    ...
    const ccgConfig: CcgConfig = new CcgConfig({ clientId: getEnvVar("CLIENT_ID"), clientSecret: getEnvVar("CLIENT_SECRET"), enterpriseId: enterpriseId, userId: userId });
...
test("test_jwt_auth", async function test_jwt_auth(): Promise<any> {
    ...
    const newUser: UserFull = await enterpriseClient.users.getUserMe({ fields: ["enterprise" as string] } satisfies GetUserMeQueryParams);
...
test("test_jwt_auth_downscope", async function test_jwt_auth_downscope(): Promise<any> {
    ...
    const uploadedFiles: Files = await parentClient.uploads.uploadFile({ attributes: { name: getUuid(), parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(1024 * 1024) } satisfies UploadFileRequestBody);
    const file: FileFull = uploadedFiles.entries![0];
    const resourcePath: string = "".concat("https://api.box.com/2.0/files/", file.id) as string;
    const downscopedToken: AccessToken = await auth.downscopeToken(["item_rename", "item_preview"], resourcePath);
    if (!!(downscopedToken.accessToken == void 0)) {
        throw new Error("Assertion failed");
    }
    const downscopedClient: BoxClient = new BoxClient({ auth: new BoxDeveloperTokenAuth({ token: downscopedToken.accessToken! }) });
    await downscopedClient.files.updateFileById(file.id, { requestBody: { name: getUuid() } satisfies UpdateFileByIdRequestBody } satisfies UpdateFileByIdOptionalsInput)
    await expect(async () => { await downscopedClient.files.deleteFileById(file.id); }).rejects.toThrow();
...
test("test_jwt_auth_revoke", async function test_jwt_auth_revoke(): Promise<any> {
    ...
    if (!!(tokenFromStorageBeforeRevoke.accessToken == tokenFromStorageAfterRevoke.accessToken)) {
        throw new Error("Assertion failed");
    }
});
test("test_oauth_auth_authorizeUrl", function test_oauth_auth_authorizeUrl(): any {
    const config: OAuthConfig = new OAuthConfig({ clientId: "OAUTH_CLIENT_ID", clientSecret: "OAUTH_CLIENT_SECRET" });
    const auth: BoxOAuth = new BoxOAuth({ config: config });
    const authUrl: string = auth.getAuthorizeUrl();
    if (!(authUrl == "https://account.box.com/api/oauth2/authorize?client_id=OAUTH_CLIENT_ID&response_type=code" || authUrl == "https://account.box.com/api/oauth2/authorize?response_type=code&client_id=OAUTH_CLIENT_ID")) {
        throw new Error("Assertion failed");
    }
});
...
test("test_ccg_auth_downscope", async function test_ccg_auth_downscope(): Promise<any> {
    const ccgConfig: CcgConfig = new CcgConfig({ clientId: getEnvVar("CLIENT_ID"), clientSecret: getEnvVar("CLIENT_SECRET"), userId: getEnvVar("USER_ID") });
    const auth: BoxCcgAuth = new BoxCcgAuth({ config: ccgConfig });
    const parentClient: BoxClient = new BoxClient({ auth: auth });
    const folder: FolderFull = await parentClient.folders.createFolder({ name: getUuid(), parent: { id: "0" } satisfies CreateFolderRequestBodyParentField } satisfies CreateFolderRequestBody);
    const resourcePath: string = "".concat("https://api.box.com/2.0/folders/", folder.id) as string;
    const downscopedToken: AccessToken = await auth.downscopeToken(["item_rename", "item_preview"], resourcePath);
    if (!!(downscopedToken.accessToken == void 0)) {
        throw new Error("Assertion failed");
    }
    const downscopedClient: BoxClient = new BoxClient({ auth: new BoxDeveloperTokenAuth({ token: downscopedToken.accessToken! }) });
    await downscopedClient.folders.updateFolderById(folder.id, { requestBody: { name: getUuid() } satisfies UpdateFolderByIdRequestBody } satisfies UpdateFolderByIdOptionalsInput)
    await expect(async () => { await downscopedClient.folders.deleteFolderById(folder.id); }).rejects.toThrow();
...
test("test_developer_token_auth_revoke", async function test_developer_token_auth_revoke(): Promise<any> {
    const developerTokenConfig: DeveloperTokenConfig = { clientId: getEnvVar("CLIENT_ID"), clientSecret: getEnvVar("CLIENT_SECRET") } satisfies DeveloperTokenConfig;
    const token: AccessToken = await getAccessToken();
    const auth: BoxDeveloperTokenAuth = new BoxDeveloperTokenAuth({ token: token.accessToken!, config: developerTokenConfig });
    const tokenFromStorageBeforeRevoke: AccessToken = await auth.retrieveToken();
    if (!!(tokenFromStorageBeforeRevoke == void 0)) {
        throw new Error("Assertion failed");
    }
    await auth.revokeToken()
    await expect(async () => { await auth.retrieveToken(); }).rejects.toThrow();
});
...
test("test_developer_token_auth", async function test_developer_token_auth(): Promise<any> {
    const userId: string = getEnvVar("USER_ID");
    const token: AccessToken = await getAccessToken();
    const devAuth: BoxDeveloperTokenAuth = new BoxDeveloperTokenAuth({ token: token.accessToken! });
...
test("test_oauth_auth_revoke", async function test_oauth_auth_revoke(): Promise<any> {
    const token: AccessToken = await getAccessToken();
    const tokenStorage: InMemoryTokenStorage = new InMemoryTokenStorage({ token: token });
    const config: OAuthConfig = new OAuthConfig({ clientId: getEnvVar("CLIENT_ID"), clientSecret: getEnvVar("CLIENT_SECRET"), tokenStorage: tokenStorage });
    const auth: BoxOAuth = new BoxOAuth({ config: config });
    const client: BoxClient = new BoxClient({ auth: auth });
    await client.users.getUserMe()
    await auth.revokeToken()
    await expect(async () => { await client.users.getUserMe(); }).rejects.toThrow();
});
