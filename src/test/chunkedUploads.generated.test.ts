...
async function reducerById(accInput: TestPartAccumulatorInput, chunk: ByteStream): Promise<TestPartAccumulator> {
    const acc: TestPartAccumulator = new TestPartAccumulator({ lastIndex: accInput.lastIndex, parts: accInput.parts, fileSize: accInput.fileSize, uploadPartUrl: accInput.uploadPartUrl, uploadSessionId: accInput.uploadSessionId, fileHash: accInput.fileHash });
    ...
    const uploadedPart: UploadedPart = await client.chunkedUploads.uploadFilePart(acc.uploadSessionId, generateByteStreamFromBuffer(chunkBuffer), { digest: digest, contentRange: contentRange } satisfies UploadFilePartHeadersInput);
    ...
    return new TestPartAccumulator({ lastIndex: bytesEnd, parts: parts.concat([part]), fileSize: acc.fileSize, uploadSessionId: acc.uploadSessionId, fileHash: acc.fileHash });
}
async function reducerByUrl(accInput: TestPartAccumulatorInput, chunk: ByteStream): Promise<TestPartAccumulator> {
    const acc: TestPartAccumulator = new TestPartAccumulator({ lastIndex: accInput.lastIndex, parts: accInput.parts, fileSize: accInput.fileSize, uploadPartUrl: accInput.uploadPartUrl, uploadSessionId: accInput.uploadSessionId, fileHash: accInput.fileHash });
    const lastIndex: number = acc.lastIndex;
    const parts: readonly UploadPart[] = acc.parts;
    const chunkBuffer: Buffer = await readByteStream(chunk);
    ...
    const bytesEnd: number = lastIndex + chunkSize;
    const contentRange: string = "".concat("bytes ", (toString(bytesStart) as string)!, "-", (toString(bytesEnd) as string)!, "/", (toString(acc.fileSize) as string)!) as string;
    const uploadedPart: UploadedPart = await client.chunkedUploads.uploadFilePartByUrl(acc.uploadPartUrl, generateByteStreamFromBuffer(chunkBuffer), { digest: digest, contentRange: contentRange } satisfies UploadFilePartByUrlHeadersInput);
    const part: UploadPart = uploadedPart.part!;
    const partSha1: string = hexToBase64(part.sha1!);
    if (!(partSha1 == sha1)) {
        throw new Error("Assertion failed");
    ...
    await acc.fileHash.updateHash(chunkBuffer)
    return new TestPartAccumulator({ lastIndex: bytesEnd, parts: parts.concat([part]), fileSize: acc.fileSize, uploadPartUrl: acc.uploadPartUrl, fileHash: acc.fileHash });
}
test("testChunkedManualProcessById", async function testChunkedManualProcessById(): Promise<any> {
    ...
    const uploadSession: UploadSession = await client.chunkedUploads.createFileUploadSession({ fileName: fileName, fileSize: fileSize, folderId: parentFolderId } satisfies CreateFileUploadSessionRequestBody);
    ...
    const chunksIterator: Iterator = iterateChunks(fileByteStream, partSize, fileSize);
    const results: TestPartAccumulator = await reduceIterator(chunksIterator, reducerById, new TestPartAccumulator({ lastIndex: -1, parts: [], fileSize: fileSize, uploadSessionId: uploadSessionId, fileHash: fileHash }));
    const parts: readonly UploadPart[] = results.parts;
    const processedSessionParts: UploadParts = await client.chunkedUploads.getFileUploadSessionParts(uploadSessionId);
    if (!(processedSessionParts.totalCount! == totalParts)) {
        throw new Error("Assertion failed");
    }
    const processedSession: UploadSession = await client.chunkedUploads.getFileUploadSessionById(uploadSessionId);
    if (!(processedSession.id! == uploadSessionId)) {
        throw new Error("Assertion failed");
    }
    const sha1: string = await fileHash.digestHash("base64");
    const digest: string = "".concat("sha=", sha1) as string;
    const committedSession: undefined | Files = await client.chunkedUploads.createFileUploadSessionCommit(uploadSessionId, { parts: parts } satisfies CreateFileUploadSessionCommitRequestBody, { digest: digest } satisfies CreateFileUploadSessionCommitHeadersInput);
...
test("testChunkedManualProcessByUrl", async function testChunkedManualProcessByUrl(): Promise<any> {
    ...
    const fileHash: Hash = new Hash({ algorithm: "sha1" as HashName });
    const chunksIterator: Iterator = iterateChunks(fileByteStream, partSize, fileSize);
    const results: TestPartAccumulator = await reduceIterator(chunksIterator, reducerByUrl, new TestPartAccumulator({ lastIndex: -1, parts: [], fileSize: fileSize, uploadPartUrl: uploadPartUrl, fileHash: fileHash }));
    const parts: readonly UploadPart[] = results.parts;
    const processedSessionParts: UploadParts = await client.chunkedUploads.getFileUploadSessionPartsByUrl(listPartsUrl);
    if (!(processedSessionParts.totalCount! == totalParts)) {
        throw new Error("Assertion failed");
    }
    const processedSession: UploadSession = await client.chunkedUploads.getFileUploadSessionByUrl(statusUrl);
    if (!(processedSession.id! == uploadSessionId)) {
        throw new Error("Assertion failed");
    }
    const sha1: string = await fileHash.digestHash("base64");
    const digest: string = "".concat("sha=", sha1) as string;
    const committedSession: undefined | Files = await client.chunkedUploads.createFileUploadSessionCommitByUrl(commitUrl, { parts: parts } satisfies CreateFileUploadSessionCommitByUrlRequestBody, { digest: digest } satisfies CreateFileUploadSessionCommitByUrlHeadersInput);
    if (!(committedSession!.entries![0].name! == fileName)) {
        throw new Error("Assertion failed");
    }
...
test("testChunkedUploadConvenienceMethod", async function testChunkedUploadConvenienceMethod(): Promise<any> {
    ...
    const fileName: string = getUuid();
    const parentFolderId: string = "0";
    const uploadedFile: File = await client.chunkedUploads.uploadBigFile(fileByteStream, fileName, fileSize, parentFolderId);
