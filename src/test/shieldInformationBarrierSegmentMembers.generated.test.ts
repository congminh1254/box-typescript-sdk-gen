...
test("testShieldInformationBarrierSegmentMembers", async function testShieldInformationBarrierSegmentMembers(): Promise<any> {
    ...
    const segmentMembers: ShieldInformationBarrierSegmentMembers = await client.shieldInformationBarrierSegmentMembers.getShieldInformationBarrierSegmentMembers({ shieldInformationBarrierSegmentId: segment.id! } satisfies GetShieldInformationBarrierSegmentMembersQueryParams);
    if (!(segmentMembers.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const segmentMemberGet: ShieldInformationBarrierSegmentMember = await client.shieldInformationBarrierSegmentMembers.getShieldInformationBarrierSegmentMemberById(segmentMember.id!);
    ...
    await expect(async () => { await client.shieldInformationBarrierSegmentMembers.getShieldInformationBarrierSegmentMemberById(segmentMember.id!); }).rejects.toThrow();
