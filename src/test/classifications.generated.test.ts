...
test("testClassifications", async function testClassifications(): Promise<any> {
    ...
    const classificationTemplateWithUpdatedClassification: ClassificationTemplate = await client.classifications.updateClassification([new UpdateClassificationRequestBody({ enumOptionKey: classification.key, data: { key: updatedClassificationName, staticConfig: { classification: { colorId: 2, classificationDefinition: updatedClassificationDescription } satisfies UpdateClassificationRequestBodyDataStaticConfigClassificationField } satisfies UpdateClassificationRequestBodyDataStaticConfigField } satisfies UpdateClassificationRequestBodyDataField })]);
    const updatedClassifications: readonly ClassificationTemplateFieldsOptionsField[] = classificationTemplateWithUpdatedClassification.fields[0].options;
    const updatedClassification: ClassificationTemplateFieldsOptionsField = updatedClassifications[0];
    ...
    if (!(updatedClassification.staticConfig!.classification!.classificationDefinition == updatedClassificationDescription)) {
        throw new Error("Assertion failed");
    }
});
export {};
