...
test("testMetadataTemplates", async function testMetadataTemplates(): Promise<any> {
    ...
    const getMetadataTemplate: MetadataTemplate = await client.metadataTemplates.getMetadataTemplateById(template.id);
    if (!(getMetadataTemplate.id == template.id)) {
        throw new Error("Assertion failed");
    }
    const getMetadataTemplateSchema: MetadataTemplate = await client.metadataTemplates.getMetadataTemplate("enterprise" as GetMetadataTemplateScope, template.templateKey!);
    if (!(getMetadataTemplateSchema.id == template.id)) {
        throw new Error("Assertion failed");
    }
    const enterpriseMetadataTemplates: MetadataTemplates = await client.metadataTemplates.getEnterpriseMetadataTemplates();
    if (!(enterpriseMetadataTemplates.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const globalMetadataTemplates: MetadataTemplates = await client.metadataTemplates.getGlobalMetadataTemplates();
    ...
    await expect(async () => { await client.metadataTemplates.deleteMetadataTemplate("enterprise" as DeleteMetadataTemplateScope, template.templateKey!); }).rejects.toThrow();
});
test("testGetMetadataTemplateByInstance", async function testGetMetadataTemplateByInstance(): Promise<any> {
    ...
    const templateKey: string = "".concat("key", getUuid()) as string;
    const template: MetadataTemplate = await client.metadataTemplates.createMetadataTemplate({ scope: "enterprise", displayName: templateKey, templateKey: templateKey, fields: [{ type: "string" as CreateMetadataTemplateRequestBodyFieldsTypeField, key: "testName", displayName: "testName" } satisfies CreateMetadataTemplateRequestBodyFieldsField] } satisfies CreateMetadataTemplateRequestBody);
    const createdMetadataInstance: MetadataFull = await client.fileMetadata.createFileMetadataById(file.id, "enterprise" as CreateFileMetadataByIdScope, templateKey, { ["testName"]: "xyz" });
    const metadataTemplates: MetadataTemplates = await client.metadataTemplates.getMetadataTemplatesByInstanceId({ metadataInstanceId: createdMetadataInstance.id! } satisfies GetMetadataTemplatesByInstanceIdQueryParams);
