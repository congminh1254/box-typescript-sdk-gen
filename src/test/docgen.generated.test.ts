...
test("testDocgenBatchAndJobs", async function testDocgenBatchAndJobs(): Promise<any> {
    ...
    const createdDocgenTemplate: DocGenTemplateBaseV2025R0 = await client.docgenTemplate.createDocgenTemplateV2025R0({ file: new FileReferenceV2025R0({ id: uploadedFile.id }) } satisfies DocGenTemplateCreateRequestV2025R0);
    const docgenBatch: DocGenBatchBaseV2025R0 = await client.docgen.createDocgenBatchV2025R0({ file: new FileReferenceV2025R0({ id: uploadedFile.id }), inputSource: "api", destinationFolder: new DocGenBatchCreateRequestV2025R0DestinationFolderField({ id: folder.id }), outputType: "pdf", documentGenerationData: [{ generatedFileName: "test", userInput: { ["abc"]: "xyz" } } satisfies DocGenDocumentGenerationDataV2025R0] } satisfies DocGenBatchCreateRequestV2025R0);
    if (!!(docgenBatch.id == "")) {
    ...
    const docgenBatchJobs: DocGenJobsV2025R0 = await client.docgen.getDocgenBatchJobByIdV2025R0(docgenBatch.id);
    ...
    if (!(toString(docgenBatchJobs.entries![0].type) as string == "docgen_job")) {
        throw new Error("Assertion failed");
    }
    ...
    const docgenJobs: DocGenJobsFullV2025R0 = await client.docgen.getDocgenJobsV2025R0();
    ...
    if (!(toString(docgenJobs.entries![0].templateFile.type) as string == "file")) {
        throw new Error("Assertion failed");
    }
    if (!!(docgenJobs.entries![0].templateFile.id == "")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(docgenJobs.entries![0].templateFileVersion.type) as string == "file_version")) {
        throw new Error("Assertion failed");
    }
    ...
    const docgenJob: DocGenJobV2025R0 = await client.docgen.getDocgenJobByIdV2025R0(docgenJobs.entries![0].id);
    ...
    if (!(toString(docgenJob.templateFileVersion.type) as string == "file_version")) {
        throw new Error("Assertion failed");
    }
