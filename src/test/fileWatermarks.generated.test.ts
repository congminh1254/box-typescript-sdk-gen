...
test("testCreateGetDeleteFileWatermark", async function testCreateGetDeleteFileWatermark(): Promise<any> {
    ...
    const uploadedFiles: Files = await client.uploads.uploadFile({ attributes: { name: fileName, parent: { id: "0" } satisfies UploadFileRequestBodyAttributesParentField } satisfies UploadFileRequestBodyAttributesField, file: generateByteStream(10) } satisfies UploadFileRequestBody);
    const file: FileFull = uploadedFiles.entries![0];
    const createdWatermark: Watermark = await client.fileWatermarks.updateFileWatermark(file.id, { watermark: new UpdateFileWatermarkRequestBodyWatermarkField({ imprint: "default" as UpdateFileWatermarkRequestBodyWatermarkImprintField }) } satisfies UpdateFileWatermarkRequestBody);
    const watermark: Watermark = await client.fileWatermarks.getFileWatermark(file.id);
    await client.fileWatermarks.deleteFileWatermark(file.id)
    await expect(async () => { await client.fileWatermarks.getFileWatermark(file.id); }).rejects.toThrow();
