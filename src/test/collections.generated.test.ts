...
test("testCollections", async function testCollections(): Promise<any> {
    ...
    const favouriteCollection: Collection = await client.collections.getCollectionById(collections.entries![0].id!);
    if (!(toString(favouriteCollection.type!) as string == "collection")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(favouriteCollection.collectionType!) as string == "favorites")) {
        throw new Error("Assertion failed");
    }
    const collectionItems: ItemsOffsetPaginated = await client.collections.getCollectionItems(favouriteCollection.id!);
    ...
    await client.folders.updateFolderById(folder.id, { requestBody: { collections: [{ id: favouriteCollection.id } satisfies UpdateFolderByIdRequestBodyCollectionsField] } satisfies UpdateFolderByIdRequestBody } satisfies UpdateFolderByIdOptionalsInput)
    const collectionItemsAfterUpdate: ItemsOffsetPaginated = await client.collections.getCollectionItems(favouriteCollection.id!);
    if (!(collectionItemsAfterUpdate.totalCount! == collectionItems.totalCount! + 1)) {
        throw new Error("Assertion failed");
    }
    await client.folders.updateFolderById(folder.id, { requestBody: { collections: [] } satisfies UpdateFolderByIdRequestBody } satisfies UpdateFolderByIdOptionalsInput)
    const collectionItemsAfterRemove: ItemsOffsetPaginated = await client.collections.getCollectionItems(favouriteCollection.id!);
