...
test("testCreateUpdateGetDeleteRetentionPolicy", async function testCreateUpdateGetDeleteRetentionPolicy(): Promise<any> {
    ...
    const retentionPolicyById: RetentionPolicy = await client.retentionPolicies.getRetentionPolicyById(retentionPolicy.id);
    if (!(retentionPolicyById.id == retentionPolicy.id)) {
        throw new Error("Assertion failed");
    }
    const retentionPolicies: RetentionPolicies = await client.retentionPolicies.getRetentionPolicies();
    if (!(retentionPolicies.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const updatedRetentionPolicyName: string = getUuid();
    const updatedRetentionPolicy: RetentionPolicy = await client.retentionPolicies.updateRetentionPolicyById(retentionPolicy.id, { requestBody: { policyName: updatedRetentionPolicyName } satisfies UpdateRetentionPolicyByIdRequestBody } satisfies UpdateRetentionPolicyByIdOptionalsInput);
