...
test("testDevicePinners", async function testDevicePinners(): Promise<any> {
    ...
    const devicePinners: DevicePinners = await client.devicePinners.getEnterpriseDevicePinners(enterpriseId);
    if (!(devicePinners.entries!.length >= 0)) {
        throw new Error("Assertion failed");
    }
    const devicePinnersInDescDirection: DevicePinners = await client.devicePinners.getEnterpriseDevicePinners(enterpriseId, { queryParams: { direction: "desc" as GetEnterpriseDevicePinnersQueryParamsDirectionField } satisfies GetEnterpriseDevicePinnersQueryParams } satisfies GetEnterpriseDevicePinnersOptionalsInput);
    if (!(devicePinnersInDescDirection.entries!.length >= 0)) {
        throw new Error("Assertion failed");
    }
    const devicePinnerId: string = "123456";
    await expect(async () => { await client.devicePinners.getDevicePinnerById(devicePinnerId); }).rejects.toThrow();
    await expect(async () => { await client.devicePinners.deleteDevicePinnerById(devicePinnerId); }).rejects.toThrow();
});
export {};
