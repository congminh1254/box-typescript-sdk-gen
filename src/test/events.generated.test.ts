...
test("testEventUpload", async function testEventUpload(): Promise<any> {
    const events: Events = await client.events.getEvents({ streamType: "admin_logs" as GetEventsQueryParamsStreamTypeField, eventType: ["UPLOAD" as GetEventsQueryParamsEventTypeField] } satisfies GetEventsQueryParams);
    if (!(events.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const firstEvent: Event = events.entries![0];
    ...
    if (!(toString(source.itemType) as string == "file" || toString(source.itemType) as string == "folder")) {
        throw new Error("Assertion failed");
    }
    if (!!(source.itemId == "")) {
        throw new Error("Assertion failed");
...
test("testEventDeleteUser", async function testEventDeleteUser(): Promise<any> {
    const events: Events = await client.events.getEvents({ streamType: "admin_logs" as GetEventsQueryParamsStreamTypeField, eventType: ["DELETE_USER" as GetEventsQueryParamsEventTypeField] } satisfies GetEventsQueryParams);
    if (!(events.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const firstEvent: Event = events.entries![0];
...
test("testEventSourceFileOrFolder", async function testEventSourceFileOrFolder(): Promise<any> {
    const events: Events = await client.events.getEvents({ streamType: "changes" as GetEventsQueryParamsStreamTypeField } satisfies GetEventsQueryParams);
    if (!(events.entries!.length > 0)) {
        throw new Error("Assertion failed");
    }
    const firstEvent: Event = events.entries![0];
    const source: File = firstEvent.source! as File;
    if (!(toString(source.type) as string == "file" || toString(source.type) as string == "folder")) {
        throw new Error("Assertion failed");
    }
    if (!!(source.id == "")) {
        throw new Error("Assertion failed");
...
test("testGetEventsWithLongPolling", async function testGetEventsWithLongPolling(): Promise<any> {
    const servers: RealtimeServers = await client.events.getEventsWithLongPolling();
...
test("testGetEventsWithDateFilters", async function testGetEventsWithDateFilters(): Promise<any> {
    ...
    const createdBeforeDate: DateTime = dateTimeFromString("2025-06-09T00:00:00Z");
    const servers: Events = await client.events.getEvents({ streamType: "admin_logs" as GetEventsQueryParamsStreamTypeField, limit: 1, createdAfter: createdAfterDate, createdBefore: createdBeforeDate } satisfies GetEventsQueryParams);
