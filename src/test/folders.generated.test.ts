...
test("test_get_folder_full_info_with_extra_fields", async function test_get_folder_full_info_with_extra_fields(): Promise<any> {
    const rootFolder: FolderFull = await client.folders.getFolderById("0", { queryParams: { fields: ["has_collaborations" as string, "tags" as string] } satisfies GetFolderByIdQueryParams } satisfies GetFolderByIdOptionalsInput);
    if (!(rootFolder.id == "0")) {
        throw new Error("Assertion failed");
    }
...
test("test_create_and_delete_folder", async function test_create_and_delete_folder(): Promise<any> {
    ...
    const createdFolder: FolderFull = await client.folders.getFolderById(newFolder.id);
    if (!(createdFolder.name == newFolderName)) {
        throw new Error("Assertion failed");
    }
    await client.folders.deleteFolderById(newFolder.id)
    await expect(async () => { await client.folders.getFolderById(newFolder.id); }).rejects.toThrow();
});
test("test_update_folder", async function test_update_folder(): Promise<any> {
    const folderToUpdateName: string = getUuid();
    const folderToUpdate: FolderFull = await client.folders.createFolder({ name: folderToUpdateName, parent: { id: "0" } satisfies CreateFolderRequestBodyParentField } satisfies CreateFolderRequestBody);
    const updatedName: string = getUuid();
    const updatedFolder: FolderFull = await client.folders.updateFolderById(folderToUpdate.id, { requestBody: { name: updatedName, description: "Updated description" } satisfies UpdateFolderByIdRequestBody } satisfies UpdateFolderByIdOptionalsInput);
...
test("test_copy_move_folder_and_list_folder_items", async function test_copy_move_folder_and_list_folder_items(): Promise<any> {
    ...
    const folderOrigin: FolderFull = await client.folders.createFolder({ name: folderOriginName, parent: { id: "0" } satisfies CreateFolderRequestBodyParentField } satisfies CreateFolderRequestBody);
    const copiedFolderName: string = getUuid();
    const copiedFolder: FolderFull = await client.folders.copyFolder(folderOrigin.id, { parent: { id: "0" } satisfies CopyFolderRequestBodyParentField, name: copiedFolderName } satisfies CopyFolderRequestBody);
    if (!(copiedFolder.parent!.id == "0")) {
        throw new Error("Assertion failed");
    }
    const movedFolderName: string = getUuid();
    const movedFolder: FolderFull = await client.folders.updateFolderById(copiedFolder.id, { requestBody: { parent: { id: folderOrigin.id } satisfies UpdateFolderByIdRequestBodyParentField, name: movedFolderName } satisfies UpdateFolderByIdRequestBody } satisfies UpdateFolderByIdOptionalsInput);
    if (!(movedFolder.parent!.id == folderOrigin.id)) {
        throw new Error("Assertion failed");
    }
    const folderItems: Items = await client.folders.getFolderItems(folderOrigin.id);
