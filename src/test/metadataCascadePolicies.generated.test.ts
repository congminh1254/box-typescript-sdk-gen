...
test("testMetadataCascadePolicies", async function testMetadataCascadePolicies(): Promise<any> {
    ...
    const cascadePolicy: MetadataCascadePolicy = await client.metadataCascadePolicies.createMetadataCascadePolicy({ folderId: folder.id, scope: "enterprise" as CreateMetadataCascadePolicyRequestBodyScopeField, templateKey: templateKey } satisfies CreateMetadataCascadePolicyRequestBody);
    if (!(toString(cascadePolicy.type) as string == "metadata_cascade_policy")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(cascadePolicy.ownerEnterprise!.type!) as string == "enterprise")) {
        throw new Error("Assertion failed");
    }
    if (!(toString(cascadePolicy.ownerEnterprise!.id!) as string == enterpriseId)) {
        throw new Error("Assertion failed");
    }
    ...
    if (!(toString(cascadePolicy.scope!) as string == "".concat("enterprise_", enterpriseId) as string)) {
        throw new Error("Assertion failed");
    }
    ...
    const policyFromTheApi: MetadataCascadePolicy = await client.metadataCascadePolicies.getMetadataCascadePolicyById(cascadePolicyId);
    if (!(cascadePolicyId == policyFromTheApi.id)) {
        throw new Error("Assertion failed");
    }
    const policies: MetadataCascadePolicies = await client.metadataCascadePolicies.getMetadataCascadePolicies({ folderId: folder.id } satisfies GetMetadataCascadePoliciesQueryParams);
    if (!(policies.entries!.length == 1)) {
        throw new Error("Assertion failed");
    }
    await expect(async () => { await client.metadataCascadePolicies.applyMetadataCascadePolicy(cascadePolicyId, { conflictResolution: "overwrite" as ApplyMetadataCascadePolicyRequestBodyConflictResolutionField } satisfies ApplyMetadataCascadePolicyRequestBody); }).rejects.toThrow();
    await client.folderMetadata.createFolderMetadataById(folder.id, "enterprise" as CreateFolderMetadataByIdScope, templateKey, { ["testName"]: "xyz" })
    ...
    await expect(async () => { await client.metadataCascadePolicies.getMetadataCascadePolicyById(cascadePolicyId); }).rejects.toThrow();
