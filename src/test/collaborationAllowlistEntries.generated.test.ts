...
test("testCollaborationAllowlistEntries", async function testCollaborationAllowlistEntries(): Promise<any> {
    const allowlist: CollaborationAllowlistEntries = await client.collaborationAllowlistEntries.getCollaborationWhitelistEntries();
    if (!(allowlist.entries!.length >= 0)) {
        throw new Error("Assertion failed");
    }
    const domain: string = "".concat(getUuid(), "example.com") as string;
    const newEntry: CollaborationAllowlistEntry = await client.collaborationAllowlistEntries.createCollaborationWhitelistEntry({ direction: "inbound" as CreateCollaborationWhitelistEntryRequestBodyDirectionField, domain: domain } satisfies CreateCollaborationWhitelistEntryRequestBody);
    if (!(toString(newEntry.type) as string == "collaboration_whitelist_entry")) {
        throw new Error("Assertion failed");
    }
    ...
    const entry: CollaborationAllowlistEntry = await client.collaborationAllowlistEntries.getCollaborationWhitelistEntryById(newEntry.id!);
    if (!(entry.id == newEntry.id)) {
        throw new Error("Assertion failed");
    }
    if (!(toString(entry.direction) as string == toString(newEntry.direction) as string)) {
        throw new Error("Assertion failed");
    }
    ...
    await expect(async () => { await client.collaborationAllowlistEntries.getCollaborationWhitelistEntryById(entry.id!); }).rejects.toThrow();
});
export {};
