...
test("testCreateGetDeleteFolderWatermark", async function testCreateGetDeleteFolderWatermark(): Promise<any> {
    ...
    const folder: FolderFull = await client.folders.createFolder({ name: folderName, parent: { id: "0" } satisfies CreateFolderRequestBodyParentField } satisfies CreateFolderRequestBody);
    const createdWatermark: Watermark = await client.folderWatermarks.updateFolderWatermark(folder.id, { watermark: new UpdateFolderWatermarkRequestBodyWatermarkField({ imprint: "default" as UpdateFolderWatermarkRequestBodyWatermarkImprintField }) } satisfies UpdateFolderWatermarkRequestBody);
    const watermark: Watermark = await client.folderWatermarks.getFolderWatermark(folder.id);
    await client.folderWatermarks.deleteFolderWatermark(folder.id)
    await expect(async () => { await client.folderWatermarks.getFolderWatermark(folder.id); }).rejects.toThrow();
