...
test("testAvatars", async function testAvatars(): Promise<any> {
    ...
    const createdAvatar: UserAvatar = await client.avatars.createUserAvatar(user.id, { pic: decodeBase64ByteStream("iVBORw0KGgoAAAANSUhEUgAAAQAAAAEAAQMAAABmvDolAAAAA1BMVEW10NBjBBbqAAAAH0lEQVRoge3BAQ0AAADCoPdPbQ43oAAAAAAAAAAAvg0hAAABmmDh1QAAAABJRU5ErkJggg=="), picContentType: "image/png", picFileName: "avatar.png" } satisfies CreateUserAvatarRequestBody);
    ...
    if (!(bufferEquals(await readByteStream(response), generateByteBuffer(0)) == false)) {
        throw new Error("Assertion failed");
    }
    await client.avatars.deleteUserAvatar(user.id)
    await expect(async () => { await client.avatars.getUserAvatar(user.id); }).rejects.toThrow();
});
export {};
