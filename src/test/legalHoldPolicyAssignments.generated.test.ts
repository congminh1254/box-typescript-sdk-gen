...
test("testLegalHoldPolicyAssignments", async function testLegalHoldPolicyAssignments(): Promise<any> {
    ...
    const legalHoldPolicy: LegalHoldPolicy = await client.legalHoldPolicies.createLegalHoldPolicy({ policyName: legalHoldPolicyName, description: legalHoldDescription, isOngoing: true } satisfies CreateLegalHoldPolicyRequestBody);
    const legalHoldPolicyId: string = legalHoldPolicy.id;
    const file: FileFull = await uploadNewFile();
    const fileId: string = file.id;
    const legalHoldPolicyAssignment: LegalHoldPolicyAssignment = await client.legalHoldPolicyAssignments.createLegalHoldPolicyAssignment({ policyId: legalHoldPolicyId, assignTo: { type: "file" as CreateLegalHoldPolicyAssignmentRequestBodyAssignToTypeField, id: fileId } satisfies CreateLegalHoldPolicyAssignmentRequestBodyAssignToField } satisfies CreateLegalHoldPolicyAssignmentRequestBody);
    ...
    if (!(toString(legalHoldPolicyAssignment.assignedTo!.type) as string == "file")) {
        throw new Error("Assertion failed");
    }
    const legalHoldPolicyAssignmentId: string = legalHoldPolicyAssignment.id!;
    const legalHoldPolicyAssignmentFromApi: LegalHoldPolicyAssignment = await client.legalHoldPolicyAssignments.getLegalHoldPolicyAssignmentById(legalHoldPolicyAssignmentId);
    if (!(legalHoldPolicyAssignmentFromApi.id! == legalHoldPolicyAssignmentId)) {
        throw new Error("Assertion failed");
    }
    const legalPolicyAssignments: LegalHoldPolicyAssignments = await client.legalHoldPolicyAssignments.getLegalHoldPolicyAssignments({ policyId: legalHoldPolicyId } satisfies GetLegalHoldPolicyAssignmentsQueryParams);
    if (!(legalPolicyAssignments.entries!.length == 1)) {
        throw new Error("Assertion failed");
    }
    const filesOnHold: FilesOnHold = await client.legalHoldPolicyAssignments.getLegalHoldPolicyAssignmentFileOnHold(legalHoldPolicyAssignmentId);
    ...
    await expect(async () => { await client.legalHoldPolicyAssignments.deleteLegalHoldPolicyAssignmentById(legalHoldPolicyAssignmentId); }).rejects.toThrow();
    await client.files.deleteFileById(fileId)
    try {
        await client.legalHoldPolicies.deleteLegalHoldPolicyById(legalHoldPolicyId)
    }
    catch (error) {
        console.log("".concat("Could not delete Legal Policy with id: ", legalHoldPolicyId) as string)
    }
    finally {
    }
});
