...
test("testSharedLinksAppItems", async function testSharedLinksAppItems(): Promise<any> {
    ...
    const appItem: AppItem = await client.sharedLinksAppItems.findAppItemForSharedLink({ boxapi: "".concat("shared_link=", appItemSharedLink) as string } satisfies FindAppItemForSharedLinkHeadersInput);
