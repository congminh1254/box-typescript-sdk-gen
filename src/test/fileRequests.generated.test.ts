...
test("testGetCopyUpdateDeleteFileRequest", async function testGetCopyUpdateDeleteFileRequest(): Promise<any> {
    ...
    const fileRequest: FileRequest = await client.fileRequests.getFileRequestById(fileRequestId);
    ...
    const copiedFileRequest: FileRequest = await client.fileRequests.createFileRequestCopy(fileRequestId, { folder: { id: fileRequest.folder.id, type: "folder" as FileRequestCopyRequestFolderTypeField } satisfies FileRequestCopyRequestFolderField } satisfies FileRequestCopyRequest);
    ...
    const updatedFileRequest: FileRequest = await client.fileRequests.updateFileRequestById(copiedFileRequest.id, { title: "updated title", description: "updated description" } satisfies FileRequestUpdateRequest);
    ...
    await expect(async () => { await client.fileRequests.getFileRequestById(updatedFileRequest.id); }).rejects.toThrow();
});
export {};
