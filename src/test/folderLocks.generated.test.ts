...
test("testFolderLocks", async function testFolderLocks(): Promise<any> {
    ...
    const folderLocks: FolderLocks = await client.folderLocks.getFolderLocks({ folderId: folder.id } satisfies GetFolderLocksQueryParams);
    if (!(folderLocks.entries!.length == 0)) {
        throw new Error("Assertion failed");
    }
    const folderLock: FolderLock = await client.folderLocks.createFolderLock({ folder: { id: folder.id, type: "folder" } satisfies CreateFolderLockRequestBodyFolderField, lockedOperations: { move: true, delete: true } satisfies CreateFolderLockRequestBodyLockedOperationsField } satisfies CreateFolderLockRequestBody);
    ...
    await expect(async () => { await client.folderLocks.deleteFolderLockById(folderLock.id!); }).rejects.toThrow();
    const newFolderLocks: FolderLocks = await client.folderLocks.getFolderLocks({ folderId: folder.id } satisfies GetFolderLocksQueryParams);
