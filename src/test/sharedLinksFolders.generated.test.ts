...
test("testSharedLinksFolders", async function testSharedLinksFolders(): Promise<any> {
    ...
    const folderFromApi: FolderFull = await client.sharedLinksFolders.getSharedLinkForFolder(folder.id, { fields: "shared_link" } satisfies GetSharedLinkForFolderQueryParams);
    ...
    const folderFromSharedLinkPassword: FolderFull = await userClient.sharedLinksFolders.findFolderForSharedLink({} satisfies FindFolderForSharedLinkQueryParams, { boxapi: "".concat("shared_link=", folderFromApi.sharedLink!.url, "&shared_link_password=Secret123@") as string } satisfies FindFolderForSharedLinkHeadersInput);
    if (!(folder.id == folderFromSharedLinkPassword.id)) {
        throw new Error("Assertion failed");
    }
    await expect(async () => { await userClient.sharedLinksFolders.findFolderForSharedLink({} satisfies FindFolderForSharedLinkQueryParams, { boxapi: "".concat("shared_link=", folderFromApi.sharedLink!.url, "&shared_link_password=incorrectPassword") as string } satisfies FindFolderForSharedLinkHeadersInput); }).rejects.toThrow();
    const updatedFolder: FolderFull = await client.sharedLinksFolders.updateSharedLinkOnFolder(folder.id, { sharedLink: { access: "collaborators" as UpdateSharedLinkOnFolderRequestBodySharedLinkAccessField } satisfies UpdateSharedLinkOnFolderRequestBodySharedLinkField } satisfies UpdateSharedLinkOnFolderRequestBody, { fields: "shared_link" } satisfies UpdateSharedLinkOnFolderQueryParams);
    if (!(toString(updatedFolder.sharedLink!.access) as string == "collaborators")) {
        throw new Error("Assertion failed");
    }
    ...
    const folderFromApiAfterRemove: FolderFull = await client.sharedLinksFolders.getSharedLinkForFolder(folder.id, { fields: "shared_link" } satisfies GetSharedLinkForFolderQueryParams);
