...
test("testEmailAliases", async function testEmailAliases(): Promise<any> {
    ...
    const newUser: UserFull = await client.users.createUser({ name: newUserName, login: newUserLogin } satisfies CreateUserRequestBody);
    ...
    const newAlias: EmailAlias = await client.emailAliases.createUserEmailAlias(newUser.id, { email: newAliasEmail } satisfies CreateUserEmailAliasRequestBody);
    const updatedAliases: EmailAliases = await client.emailAliases.getUserEmailAliases(newUser.id);
    ...
    const finalAliases: EmailAliases = await client.emailAliases.getUserEmailAliases(newUser.id);
