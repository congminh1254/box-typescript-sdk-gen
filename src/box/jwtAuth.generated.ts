...
export class BoxJwtAuth implements Authentication {
    ...
    constructor(fields: Omit<BoxJwtAuth, "tokenStorage" | "subjectId" | "subjectType" | "refreshToken" | "retrieveToken" | "retrieveAuthorizationHeader" | "withUserSubject" | "withEnterpriseSubject" | "downscopeToken" | "revokeToken">) {
        ...
        this.subjectId = !(this.config.enterpriseId == void 0) ? this.config.enterpriseId : this.config.userId;
        this.subjectType = !(this.config.enterpriseId == void 0) ? "enterprise" : "user";
    }
    /**
    ...
    async refreshToken(networkSession?: NetworkSession): Promise<AccessToken> {
        ...
        const alg: JwtAlgorithm = !(this.config.algorithm == void 0) ? this.config.algorithm! : "RS256" as JwtAlgorithm;
        const claims: {
            readonly [key: string]: any;
        } = { ["exp"]: getEpochTimeInSeconds() + 30, ["box_sub_type"]: this.subjectType };
        const jwtOptions: JwtSignOptions = { algorithm: alg, audience: "https://api.box.com/oauth2/token", subject: this.subjectId, issuer: this.config.clientId, jwtid: getUuid(), keyid: this.config.jwtKeyId } satisfies JwtSignOptions;
        const jwtKey: JwtKey = { key: this.config.privateKey, passphrase: this.config.privateKeyPassphrase } satisfies JwtKey;
        const assertion: string = await createJwtAssertion(claims, jwtKey, jwtOptions);
        const authManager: AuthorizationManager = new AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession! : new NetworkSession({}) });
        const token: AccessToken = await authManager.requestAccessToken({ grantType: "urn:ietf:params:oauth:grant-type:jwt-bearer" as PostOAuth2TokenGrantTypeField, assertion: assertion, clientId: this.config.clientId, clientSecret: this.config.clientSecret } satisfies PostOAuth2Token);
     ...
     * @returns {BoxJwtAuth}
     */
    withUserSubject(userId: string, tokenStorage: TokenStorage = new InMemoryTokenStorage({})): BoxJwtAuth {
        const newConfig: JwtConfig = new JwtConfig({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, enterpriseId: void 0, userId: userId, jwtKeyId: this.config.jwtKeyId, privateKey: this.config.privateKey, privateKeyPassphrase: this.config.privateKeyPassphrase, tokenStorage: tokenStorage });
     ...
     * @returns {BoxJwtAuth}
     */
    withEnterpriseSubject(enterpriseId: string, tokenStorage: TokenStorage = new InMemoryTokenStorage({})): BoxJwtAuth {
        const newConfig: JwtConfig = new JwtConfig({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, enterpriseId: enterpriseId, userId: void 0, jwtKeyId: this.config.jwtKeyId, privateKey: this.config.privateKey, privateKeyPassphrase: this.config.privateKeyPassphrase, tokenStorage: tokenStorage });
        const newAuth: BoxJwtAuth = new BoxJwtAuth({ config: newConfig });
        return newAuth;
    }
    /**
     ...
     * @returns {Promise<AccessToken>}
     */
    async downscopeToken(scopes: readonly string[], resource?: string, sharedLink?: string, networkSession?: NetworkSession): Promise<AccessToken> {
    ...
    async revokeToken(networkSession?: NetworkSession): Promise<undefined> {
        ...
        const authManager: AuthorizationManager = new AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession! : new NetworkSession({}) });
        await authManager.revokeAccessToken({ token: oldToken.accessToken, clientId: this.config.clientId, clientSecret: this.config.clientSecret } satisfies PostOAuth2Revoke)
        await this.tokenStorage.clear()
        return void 0;
    }
}
export function serializeJwtConfigAppSettingsAppAuth(val: JwtConfigAppSettingsAppAuth): SerializedData {
    return { ["publicKeyID"]: val.publicKeyId, ["privateKey"]: val.privateKey, ["passphrase"]: val.passphrase };
}
export function deserializeJwtConfigAppSettingsAppAuth(val: SerializedData): JwtConfigAppSettingsAppAuth {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"JwtConfigAppSettingsAppAuth\"" });
    }
    if (val.publicKeyID == void 0) {
        throw new BoxSdkError({ message: "Expecting \"publicKeyID\" of type \"JwtConfigAppSettingsAppAuth\" to be defined" });
    }
    if (!sdIsString(val.publicKeyID)) {
        throw new BoxSdkError({ message: "Expecting string for \"publicKeyID\" of type \"JwtConfigAppSettingsAppAuth\"" });
    }
    const publicKeyId: string = val.publicKeyID;
    if (val.privateKey == void 0) {
        throw new BoxSdkError({ message: "Expecting \"privateKey\" of type \"JwtConfigAppSettingsAppAuth\" to be defined" });
    }
    if (!sdIsString(val.privateKey)) {
        throw new BoxSdkError({ message: "Expecting string for \"privateKey\" of type \"JwtConfigAppSettingsAppAuth\"" });
    }
    const privateKey: string = val.privateKey;
    if (val.passphrase == void 0) {
        throw new BoxSdkError({ message: "Expecting \"passphrase\" of type \"JwtConfigAppSettingsAppAuth\" to be defined" });
    }
    if (!sdIsString(val.passphrase)) {
        throw new BoxSdkError({ message: "Expecting string for \"passphrase\" of type \"JwtConfigAppSettingsAppAuth\"" });
    }
    const passphrase: string = val.passphrase;
    return { publicKeyId: publicKeyId, privateKey: privateKey, passphrase: passphrase } satisfies JwtConfigAppSettingsAppAuth;
}
export function serializeJwtConfigAppSettings(val: JwtConfigAppSettings): SerializedData {
    return { ["clientID"]: val.clientId, ["clientSecret"]: val.clientSecret, ["appAuth"]: serializeJwtConfigAppSettingsAppAuth(val.appAuth) };
}
export function deserializeJwtConfigAppSettings(val: SerializedData): JwtConfigAppSettings {
    if (!sdIsMap(val)) {
        throw new BoxSdkError({ message: "Expecting a map for \"JwtConfigAppSettings\"" });
    }
    if (val.clientID == void 0) {
        throw new BoxSdkError({ message: "Expecting \"clientID\" of type \"JwtConfigAppSettings\" to be defined" });
    }
    if (!sdIsString(val.clientID)) {
        throw new BoxSdkError({ message: "Expecting string for \"clientID\" of type \"JwtConfigAppSettings\"" });
    }
    const clientId: string = val.clientID;
    if (val.clientSecret == void 0) {
        throw new BoxSdkError({ message: "Expecting \"clientSecret\" of type \"JwtConfigAppSettings\" to be defined" });
    }
    if (!sdIsString(val.clientSecret)) {
        throw new BoxSdkError({ message: "Expecting string for \"clientSecret\" of type \"JwtConfigAppSettings\"" });
    }
    const clientSecret: string = val.clientSecret;
    if (val.appAuth == void 0) {
        throw new BoxSdkError({ message: "Expecting \"appAuth\" of type \"JwtConfigAppSettings\" to be defined" });
    }
    const appAuth: JwtConfigAppSettingsAppAuth = deserializeJwtConfigAppSettingsAppAuth(val.appAuth);
    return { clientId: clientId, clientSecret: clientSecret, appAuth: appAuth } satisfies JwtConfigAppSettings;
}
export function serializeJwtConfigFile(val: JwtConfigFile): SerializedData {
    return { ["enterpriseID"]: val.enterpriseId, ["userID"]: val.userId, ["boxAppSettings"]: serializeJwtConfigAppSettings(val.boxAppSettings) };
}
export function deserializeJwtConfigFile(val: SerializedData): JwtConfigFile {
    ...
    if (!(val.enterpriseID == void 0) && !sdIsString(val.enterpriseID)) {
        throw new BoxSdkError({ message: "Expecting string for \"enterpriseID\" of type \"JwtConfigFile\"" });
    }
    const enterpriseId: undefined | string = val.enterpriseID == void 0 ? void 0 : val.enterpriseID;
    if (!(val.userID == void 0) && !sdIsString(val.userID)) {
        throw new BoxSdkError({ message: "Expecting string for \"userID\" of type \"JwtConfigFile\"" });
    }
    const userId: undefined | string = val.userID == void 0 ? void 0 : val.userID;
    if (val.boxAppSettings == void 0) {
        throw new BoxSdkError({ message: "Expecting \"boxAppSettings\" of type \"JwtConfigFile\" to be defined" });
    }
    const boxAppSettings: JwtConfigAppSettings = deserializeJwtConfigAppSettings(val.boxAppSettings);
    return { enterpriseId: enterpriseId, userId: userId, boxAppSettings: boxAppSettings } satisfies JwtConfigFile;
}
