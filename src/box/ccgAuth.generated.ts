...
export class BoxCcgAuth implements Authentication {
    ...
    constructor(fields: Omit<BoxCcgAuth, "tokenStorage" | "subjectId" | "subjectType" | "refreshToken" | "retrieveToken" | "retrieveAuthorizationHeader" | "withUserSubject" | "withEnterpriseSubject" | "downscopeToken" | "revokeToken">) {
        ...
        this.subjectId = !(this.config.userId == void 0) ? this.config.userId : this.config.enterpriseId;
        this.subjectType = !(this.config.userId == void 0) ? "user" as PostOAuth2TokenBoxSubjectTypeField : "enterprise" as PostOAuth2TokenBoxSubjectTypeField;
    }
    /**
    ...
    async refreshToken(networkSession?: NetworkSession): Promise<AccessToken> {
        ...
        const token: AccessToken = await authManager.requestAccessToken({ grantType: "client_credentials" as PostOAuth2TokenGrantTypeField, clientId: this.config.clientId, clientSecret: this.config.clientSecret, boxSubjectType: this.subjectType, boxSubjectId: this.subjectId } satisfies PostOAuth2Token);
     ...
     * @returns {BoxCcgAuth}
     */
    withUserSubject(userId: string, tokenStorage: TokenStorage = new InMemoryTokenStorage({})): BoxCcgAuth {
        const newConfig: CcgConfig = new CcgConfig({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, enterpriseId: this.config.enterpriseId, userId: userId, tokenStorage: tokenStorage });
     ...
     * @returns {BoxCcgAuth}
     */
    withEnterpriseSubject(enterpriseId: string, tokenStorage: TokenStorage = new InMemoryTokenStorage({})): BoxCcgAuth {
        const newConfig: CcgConfig = new CcgConfig({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, enterpriseId: enterpriseId, userId: void 0, tokenStorage: tokenStorage });
        return new BoxCcgAuth({ config: newConfig });
    }
    /**
    ...
    async revokeToken(networkSession?: NetworkSession): Promise<undefined> {
        ...
        const authManager: AuthorizationManager = new AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession! : new NetworkSession({}) });
        await authManager.revokeAccessToken({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, token: oldToken!.accessToken } satisfies PostOAuth2Revoke)
