...
export class BoxOAuth implements Authentication {
     ...
     * @returns {string}
     */
    getAuthorizeUrl(options: GetAuthorizeUrlOptions = {} satisfies GetAuthorizeUrlOptions): string {
        const paramsMap: {
            readonly [key: string]: string;
        } = prepareParams({ ["client_id"]: !(options.clientId == void 0) ? options.clientId : this.config.clientId, ["response_type"]: !(options.responseType == void 0) ? options.responseType : "code", ["redirect_uri"]: options.redirectUri, ["state"]: options.state, ["scope"]: options.scope });
        return "".concat("https://account.box.com/api/oauth2/authorize?", sdToUrlParams(JSON.stringify(paramsMap) as SerializedData)) as string;
    }
    /**
     ...
     */
    async getTokensAuthorizationCodeGrant(authorizationCode: string, networkSession?: NetworkSession): Promise<AccessToken> {
        ...
        const token: AccessToken = await authManager.requestAccessToken({ grantType: "authorization_code" as PostOAuth2TokenGrantTypeField, clientId: this.config.clientId, clientSecret: this.config.clientSecret, code: authorizationCode } satisfies PostOAuth2Token);
    ...
    async refreshToken(networkSession?: NetworkSession): Promise<AccessToken> {
        ...
        const tokenUsedForRefresh: undefined | string = !(oldToken == void 0) ? oldToken!.refreshToken : void 0;
        const authManager: AuthorizationManager = new AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession! : new NetworkSession({}) });
        const token: AccessToken = await authManager.requestAccessToken({ grantType: "refresh_token" as PostOAuth2TokenGrantTypeField, clientId: this.config.clientId, clientSecret: this.config.clientSecret, refreshToken: tokenUsedForRefresh } satisfies PostOAuth2Token);
        await this.tokenStorage.store(token)
        return token;
    }
    /**
    ...
    async revokeToken(networkSession?: NetworkSession): Promise<undefined> {
        ...
        const authManager: AuthorizationManager = new AuthorizationManager({ networkSession: !(networkSession == void 0) ? networkSession! : new NetworkSession({}) });
        await authManager.revokeAccessToken({ clientId: this.config.clientId, clientSecret: this.config.clientSecret, token: token!.accessToken } satisfies PostOAuth2Revoke)
        return void 0;
    }
    /**
