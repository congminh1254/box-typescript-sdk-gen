...
export class GetStoragePolicyByIdOptionals {
    readonly headers: GetStoragePolicyByIdHeaders = new GetStoragePolicyByIdHeaders({});
...
export class StoragePoliciesManager {
     ...
     */
    async getStoragePolicies(queryParams: GetStoragePoliciesQueryParams = {} satisfies GetStoragePoliciesQueryParams, headersInput: GetStoragePoliciesHeadersInput = new GetStoragePoliciesHeaders({}), cancellationToken?: CancellationToken): Promise<StoragePolicies> {
        const headers: GetStoragePoliciesHeaders = new GetStoragePoliciesHeaders({ extraHeaders: headersInput.extraHeaders });
        const queryParamsMap: {
            readonly [key: string]: string;
        } = prepareParams({ ["fields"]: queryParams.fields ? queryParams.fields.map(toString).join(",") : undefined, ["marker"]: toString(queryParams.marker) as string, ["limit"]: toString(queryParams.limit) as string });
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/storage_policies") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
     ...
     */
    async getStoragePolicyById(storagePolicyId: string, optionalsInput: GetStoragePolicyByIdOptionalsInput = {}): Promise<StoragePolicy> {
        const optionals: GetStoragePolicyByIdOptionals = new GetStoragePolicyByIdOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/storage_policies/", toString(storagePolicyId) as string) as string, method: "GET", headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
