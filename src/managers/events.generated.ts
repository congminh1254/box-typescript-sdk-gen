...
export class EventsManager {
     ...
     */
    async getEventsWithLongPolling(headersInput: GetEventsWithLongPollingHeadersInput = new GetEventsWithLongPollingHeaders({}), cancellationToken?: CancellationToken): Promise<RealtimeServers> {
        const headers: GetEventsWithLongPollingHeaders = new GetEventsWithLongPollingHeaders({ extraHeaders: headersInput.extraHeaders });
        const headersMap: {
     ...
     */
    async getEvents(queryParams: GetEventsQueryParams = {} satisfies GetEventsQueryParams, headersInput: GetEventsHeadersInput = new GetEventsHeaders({}), cancellationToken?: CancellationToken): Promise<Events> {
        const headers: GetEventsHeaders = new GetEventsHeaders({ extraHeaders: headersInput.extraHeaders });
        const queryParamsMap: {
            readonly [key: string]: string;
        } = prepareParams({ ["stream_type"]: toString(queryParams.streamType) as string, ["stream_position"]: toString(queryParams.streamPosition) as string, ["limit"]: toString(queryParams.limit) as string, ["event_type"]: queryParams.eventType ? queryParams.eventType.map(toString).join(",") : undefined, ["created_after"]: queryParams.createdAfter ? serializeDateTime(queryParams.createdAfter) : undefined, ["created_before"]: queryParams.createdBefore ? serializeDateTime(queryParams.createdBefore) : undefined });
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/events") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
...
export function serializeGetEventsQueryParamsStreamTypeField(val: GetEventsQueryParamsStreamTypeField): SerializedData {
    return val;
}
export function deserializeGetEventsQueryParamsStreamTypeField(val: SerializedData): GetEventsQueryParamsStreamTypeField {
    if (val == "all") {
        return val;
    }
    if (val == "changes") {
    ...
    throw new BoxSdkError({ message: "Can't deserialize GetEventsQueryParamsStreamTypeField" });
}
export function serializeGetEventsQueryParamsEventTypeField(val: GetEventsQueryParamsEventTypeField): SerializedData {
    return val;
}
export function deserializeGetEventsQueryParamsEventTypeField(val: SerializedData): GetEventsQueryParamsEventTypeField {
    ...
    throw new BoxSdkError({ message: "Can't deserialize GetEventsQueryParamsEventTypeField" });
}
