...
export class GetDownloadFileUrlOptionals {
    readonly queryParams: GetDownloadFileUrlQueryParams = {} satisfies GetDownloadFileUrlQueryParams;
...
export class DownloadFileOptionals {
    readonly queryParams: DownloadFileQueryParams = {} satisfies DownloadFileQueryParams;
...
export class DownloadsManager {
     ...
     */
    async getDownloadFileUrl(fileId: string, optionalsInput: GetDownloadFileUrlOptionalsInput = {}): Promise<string> {
        const optionals: GetDownloadFileUrlOptionals = new GetDownloadFileUrlOptionals({ queryParams: optionalsInput.queryParams, headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const queryParams: any = optionals.queryParams;
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const queryParamsMap: {
            readonly [key: string]: string;
        } = prepareParams({ ["version"]: toString(queryParams.version) as string, ["access_token"]: toString(queryParams.accessToken) as string });
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{ ["range"]: toString(headers.range) as string, ["boxapi"]: toString(headers.boxapi) as string }, ...headers.extraHeaders });
        const cancellationController: CancellationController = createCancellationController();
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/files/", toString(fileId) as string, "/content") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "no_content" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken == void 0 ? cancellationController.signal : cancellationToken, followRedirects: false }));
        if (isBrowser()) {
     ...
     */
    async downloadFile(fileId: string, optionalsInput: DownloadFileOptionalsInput = {}): Promise<undefined | ByteStream> {
        const optionals: DownloadFileOptionals = new DownloadFileOptionals({ queryParams: optionalsInput.queryParams, headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const queryParams: any = optionals.queryParams;
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const queryParamsMap: {
        ...
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/files/", toString(fileId) as string, "/content") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "binary" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
