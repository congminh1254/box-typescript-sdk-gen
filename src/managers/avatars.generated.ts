...
export class AvatarsManager {
     ...
     */
    async getUserAvatar(userId: string, optionalsInput: GetUserAvatarOptionalsInput = {}): Promise<ByteStream> {
        const optionals: GetUserAvatarOptionals = new GetUserAvatarOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/users/", toString(userId) as string, "/avatar") as string, method: "GET", headers: headersMap, responseFormat: "binary" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
     ...
     */
    async createUserAvatar(userId: string, requestBody: CreateUserAvatarRequestBody, optionalsInput: CreateUserAvatarOptionalsInput = {}): Promise<UserAvatar> {
        const optionals: CreateUserAvatarOptionals = new CreateUserAvatarOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/users/", toString(userId) as string, "/avatar") as string, method: "POST", headers: headersMap, multipartData: [{ partName: "pic", fileStream: requestBody.pic, fileName: requestBody.picFileName, contentType: requestBody.picContentType } satisfies MultipartItem], contentType: "multipart/form-data", responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
     ...
     */
    async deleteUserAvatar(userId: string, optionalsInput: DeleteUserAvatarOptionalsInput = {}): Promise<undefined> {
        const optionals: DeleteUserAvatarOptionals = new DeleteUserAvatarOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/users/", toString(userId) as string, "/avatar") as string, method: "DELETE", headers: headersMap, responseFormat: "no_content" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return void 0;
    }
}
