...
export class SharedLinksAppItemsManager {
     ...
     */
    async findAppItemForSharedLink(headersInput: FindAppItemForSharedLinkHeadersInput, optionalsInput: FindAppItemForSharedLinkOptionalsInput = {}): Promise<AppItem> {
        const headers: FindAppItemForSharedLinkHeaders = new FindAppItemForSharedLinkHeaders({ boxapi: headersInput.boxapi, extraHeaders: headersInput.extraHeaders });
        const optionals: FindAppItemForSharedLinkOptionals = new FindAppItemForSharedLinkOptionals({ cancellationToken: optionalsInput.cancellationToken });
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{ ["boxapi"]: toString(headers.boxapi) as string }, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/shared_items#app_items") as string, method: "GET", headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
