...
export class SignTemplatesManager {
     ...
     */
    async getSignTemplates(queryParams: GetSignTemplatesQueryParams = {} satisfies GetSignTemplatesQueryParams, headersInput: GetSignTemplatesHeadersInput = new GetSignTemplatesHeaders({}), cancellationToken?: CancellationToken): Promise<SignTemplates> {
        const headers: GetSignTemplatesHeaders = new GetSignTemplatesHeaders({ extraHeaders: headersInput.extraHeaders });
        ...
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/sign_templates") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
     ...
     */
    async getSignTemplateById(templateId: string, optionalsInput: GetSignTemplateByIdOptionalsInput = {}): Promise<SignTemplate> {
        const optionals: GetSignTemplateByIdOptionals = new GetSignTemplateByIdOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/sign_templates/", toString(templateId) as string) as string, method: "GET", headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
