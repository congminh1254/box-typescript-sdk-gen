...
export class GetFileVersionLegalHoldByIdOptionals {
    readonly headers: GetFileVersionLegalHoldByIdHeaders = new GetFileVersionLegalHoldByIdHeaders({});
...
export class GetFileVersionLegalHoldsOptionals {
    readonly headers: GetFileVersionLegalHoldsHeaders = new GetFileVersionLegalHoldsHeaders({});
    readonly cancellationToken?: CancellationToken = void 0;
...
export class FileVersionLegalHoldsManager {
     ...
     */
    async getFileVersionLegalHoldById(fileVersionLegalHoldId: string, optionalsInput: GetFileVersionLegalHoldByIdOptionalsInput = {}): Promise<FileVersionLegalHold> {
        const optionals: GetFileVersionLegalHoldByIdOptionals = new GetFileVersionLegalHoldByIdOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/file_version_legal_holds/", toString(fileVersionLegalHoldId) as string) as string, method: "GET", headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
     ...
     */
    async getFileVersionLegalHolds(queryParams: GetFileVersionLegalHoldsQueryParams, optionalsInput: GetFileVersionLegalHoldsOptionalsInput = {}): Promise<FileVersionLegalHolds> {
        const optionals: GetFileVersionLegalHoldsOptionals = new GetFileVersionLegalHoldsOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const queryParamsMap: {
            readonly [key: string]: string;
        } = prepareParams({ ["policy_id"]: toString(queryParams.policyId) as string, ["marker"]: toString(queryParams.marker) as string, ["limit"]: toString(queryParams.limit) as string });
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/file_version_legal_holds") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
