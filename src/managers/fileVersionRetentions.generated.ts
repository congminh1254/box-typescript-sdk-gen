...
export class GetFileVersionRetentionByIdOptionals {
    readonly headers: GetFileVersionRetentionByIdHeaders = new GetFileVersionRetentionByIdHeaders({});
...
export class FileVersionRetentionsManager {
     ...
     */
    async getFileVersionRetentions(queryParams: GetFileVersionRetentionsQueryParams = {} satisfies GetFileVersionRetentionsQueryParams, headersInput: GetFileVersionRetentionsHeadersInput = new GetFileVersionRetentionsHeaders({}), cancellationToken?: CancellationToken): Promise<FileVersionRetentions> {
        const headers: GetFileVersionRetentionsHeaders = new GetFileVersionRetentionsHeaders({ extraHeaders: headersInput.extraHeaders });
        const queryParamsMap: {
            readonly [key: string]: string;
        } = prepareParams({ ["file_id"]: toString(queryParams.fileId) as string, ["file_version_id"]: toString(queryParams.fileVersionId) as string, ["policy_id"]: toString(queryParams.policyId) as string, ["disposition_action"]: toString(queryParams.dispositionAction) as string, ["disposition_before"]: toString(queryParams.dispositionBefore) as string, ["disposition_after"]: toString(queryParams.dispositionAfter) as string, ["limit"]: toString(queryParams.limit) as string, ["marker"]: toString(queryParams.marker) as string });
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/file_version_retentions") as string, method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
     ...
     */
    async getFileVersionRetentionById(fileVersionRetentionId: string, optionalsInput: GetFileVersionRetentionByIdOptionalsInput = {}): Promise<FileVersionRetention> {
        const optionals: GetFileVersionRetentionByIdOptionals = new GetFileVersionRetentionByIdOptionals({ headers: optionalsInput.headers, cancellationToken: optionalsInput.cancellationToken });
        const headers: any = optionals.headers;
        const cancellationToken: any = optionals.cancellationToken;
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await this.networkSession.networkClient.fetch(new FetchOptions({ url: "".concat(this.networkSession.baseUrls.baseUrl, "/2.0/file_version_retentions/", toString(fileVersionRetentionId) as string) as string, method: "GET", headers: headersMap, responseFormat: "json" as ResponseFormat, auth: this.auth, networkSession: this.networkSession, cancellationToken: cancellationToken }));
        return {
...
export function serializeGetFileVersionRetentionsQueryParamsDispositionActionField(val: GetFileVersionRetentionsQueryParamsDispositionActionField): SerializedData {
    return val;
}
export function deserializeGetFileVersionRetentionsQueryParamsDispositionActionField(val: SerializedData): GetFileVersionRetentionsQueryParamsDispositionActionField {
    ...
    throw new BoxSdkError({ message: "Can't deserialize GetFileVersionRetentionsQueryParamsDispositionActionField" });
}
