...
export class BoxClient {
    ...
    readonly networkSession: NetworkSession = new NetworkSession({ baseUrls: new BaseUrls({}) });
    ...
    constructor(fields: Omit<BoxClient, "authorization" | "files" | "trashedFiles" | "appItemAssociations" | "downloads" | "uploads" | "chunkedUploads" | "listCollaborations" | "comments" | "tasks" | "fileVersions" | "fileMetadata" | "fileClassifications" | "skills" | "fileWatermarks" | "fileRequests" | "folders" | "trashedFolders" | "folderMetadata" | "folderClassifications" | "trashedItems" | "folderWatermarks" | "folderLocks" | "metadataTemplates" | "classifications" | "metadataCascadePolicies" | "search" | "userCollaborations" | "taskAssignments" | "sharedLinksFiles" | "sharedLinksFolders" | "webLinks" | "trashedWebLinks" | "sharedLinksWebLinks" | "sharedLinksAppItems" | "users" | "sessionTermination" | "avatars" | "transfer" | "emailAliases" | "memberships" | "invites" | "groups" | "webhooks" | "events" | "collections" | "recentItems" | "retentionPolicies" | "retentionPolicyAssignments" | "legalHoldPolicies" | "legalHoldPolicyAssignments" | "fileVersionRetentions" | "fileVersionLegalHolds" | "shieldInformationBarriers" | "shieldInformationBarrierReports" | "shieldInformationBarrierSegments" | "shieldInformationBarrierSegmentMembers" | "shieldInformationBarrierSegmentRestrictions" | "devicePinners" | "termsOfServices" | "termsOfServiceUserStatuses" | "collaborationAllowlistEntries" | "collaborationAllowlistExemptTargets" | "storagePolicies" | "storagePolicyAssignments" | "zipDownloads" | "signRequests" | "workflows" | "signTemplates" | "integrationMappings" | "ai" | "aiStudio" | "docgenTemplate" | "docgen" | "networkSession" | "makeRequest" | "withAsUserHeader" | "withSuppressedNotifications" | "withExtraHeaders" | "withCustomBaseUrls" | "withProxy" | "withCustomAgentOptions" | "withInterceptors"> & Partial<Pick<BoxClient, "networkSession">>) {
        ...
        this.appItemAssociations = new AppItemAssociationsManager({ auth: this.auth, networkSession: this.networkSession });
        ...
        this.sharedLinksAppItems = new SharedLinksAppItemsManager({ auth: this.auth, networkSession: this.networkSession });
        ...
        this.ai = new AiManager({ auth: this.auth, networkSession: this.networkSession });
        this.aiStudio = new AiStudioManager({ auth: this.auth, networkSession: this.networkSession });
        this.docgenTemplate = new DocgenTemplateManager({ auth: this.auth, networkSession: this.networkSession });
        this.docgen = new DocgenManager({ auth: this.auth, networkSession: this.networkSession });
    }
    /**
     ...
     */
    async makeRequest(fetchOptionsInput: FetchOptionsInput): Promise<FetchResponse> {
        const fetchOptions: FetchOptions = new FetchOptions({ url: fetchOptionsInput.url, method: fetchOptionsInput.method, params: fetchOptionsInput.params, headers: fetchOptionsInput.headers, data: fetchOptionsInput.data, fileStream: fetchOptionsInput.fileStream, multipartData: fetchOptionsInput.multipartData, contentType: fetchOptionsInput.contentType, responseFormat: fetchOptionsInput.responseFormat, auth: fetchOptionsInput.auth, networkSession: fetchOptionsInput.networkSession, cancellationToken: fetchOptionsInput.cancellationToken, followRedirects: fetchOptionsInput.followRedirects });
        const auth: Authentication = fetchOptions.auth == void 0 ? this.auth : fetchOptions.auth!;
        const networkSession: NetworkSession = fetchOptions.networkSession == void 0 ? this.networkSession : fetchOptions.networkSession!;
        const enrichedFetchOptions: FetchOptions = new FetchOptions({ auth: auth, networkSession: networkSession, url: fetchOptions.url, method: fetchOptions.method, params: fetchOptions.params, headers: fetchOptions.headers, data: fetchOptions.data, fileStream: fetchOptions.fileStream, multipartData: fetchOptions.multipartData, contentType: fetchOptions.contentType, responseFormat: fetchOptions.responseFormat, followRedirects: fetchOptions.followRedirects });
    ...
    withAsUserHeader(userId: string): BoxClient {
        return new BoxClient({ auth: this.auth, networkSession: this.networkSession.withAdditionalHeaders({ ["As-User"]: userId }) });
    }
    /**
    ...
    withSuppressedNotifications(): BoxClient {
        return new BoxClient({ auth: this.auth, networkSession: this.networkSession.withAdditionalHeaders({ ["Box-Notifications"]: "off" }) });
    }
    /**
    ...
    withCustomBaseUrls(baseUrlsInput: BaseUrlsInput): BoxClient {
        const baseUrls: BaseUrls = new BaseUrls({ baseUrl: baseUrlsInput.baseUrl, uploadUrl: baseUrlsInput.uploadUrl, oauth2Url: baseUrlsInput.oauth2Url });
        return new BoxClient({ auth: this.auth, networkSession: this.networkSession.withCustomBaseUrls(baseUrls) });
    }
    /**
    ...
    withProxy(config: ProxyConfig): BoxClient {
        return new BoxClient({ auth: this.auth, networkSession: this.networkSession.withProxy(config) });
    }
    /**
    ...
    withCustomAgentOptions(agentOptions: AgentOptions): BoxClient {
        return new BoxClient({ auth: this.auth, networkSession: this.networkSession.withCustomAgentOptions(agentOptions) });
    }
    /**
    ...
    withInterceptors(interceptors: readonly Interceptor[]): BoxClient {
        return new BoxClient({ auth: this.auth, networkSession: this.networkSession.withInterceptors(interceptors) });
    }
}
